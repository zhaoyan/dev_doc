...
\begin{itemize}
		...
		\item There are two basic shell, one is bash and the other is tcsh. You can change it in command line by command "bash" and "tcsh". You can use echo \$0 to see which shell you are using right now.
...
\begin{itemize} 
	...
	\item \linuxcommand{export DEPART=Sale} and \linuxcommand{DEPART=Sale ; export DEPART} they means the same. If the export statement in a script file, you must use \linuxcommand{Source a.sh} or \linuxcommand{. a.sh} to run the script file, then It will affect the continuous processing. source means that this command only run in current bash, it doesn't run in a child shell.  
...
\end{verbatim}
\item If There is only one enviroment variable need to be set, you can run setenv or export in your current shell directly, then you can run a command, this command will be runned in the child process, but child process can access parent enviroment varaible, it's OK

\item If There are a few enviroment varaibles need to be set, You'd better put them into a script file. \textbf{But you have to use source a.sh} to make sure a.sh running in the current process, not a child process. 

\item Two shells don't share history and enviroment varaibles. 
\end{itemize}
...
\begin{itemize}
		\item What to do if you get a new computer?
\begin{enumerate}
		\item Run \linuxcommand{clone https://github.com/VundleVim/Vundle.vim.git ~/.vim/bundle/Vundle.vim}
		\item Go to my github, downlaod my own vimrc and change it to .vimrc
...
\end{enumerate}
		...
		\item \textbf{Rule2: Once you go to normal mode, you will have a lot of commands which you combines together to finish your task, 1)basic move, 2)fast move 3)delete+(motion,text ojbects) 4)visual,copy or move. 5)specific edit(r, surrounds commands) After all necessary task,6)you can use i(I),a(A) o(O) to return back insert mode at right position.}
		...
		\item Some OS support swap ESC and Captive, I recommend this method. It's more reliable and just get used to it.  
...
\subsection{Motion command}
	...
	\begin{center}
	    ...
	    next, previous word(end) & <C-o>.. & e, ge \\
         ...
         \hline 
		 match next character &<A-q>, <A-z>& fx, Fx \\
...
\end{center}
		...
		\item h,j,k,l, w(W), e(E), b(B), \^ , \$ ,fx,tx, (,), \{ ..., gg. All these move command you can add number before them. For example, \verb=4f,= will jump to the fourth ,(comma) directly.    
				...
				\item \textbf{From 2 to 4 words, you can use f and F command to follow a letter. press ; or , repeat once or twice}
...
\begin{enumerate}
	...
	\item "w b e j k h l" are no jump command, they will not recored in jump list:
...
\item marks lists basic:
		...
		\item Lowcase letter just mark position in current file. Captive letter can mark a position in different file, when you jump to this position, you don't need to open this file.
...
\item \linuxcommand{:lv word \%} will search all word in current file. It will show all the result in position list. 
		\begin{enumerate}
				...
				\item both \# and /, you can press F4 to toggle highlight reserch result. 
				...
				\item Another subtopics is search scopes.
						\begin{enumerate}
								\item / search current buffer, it support regex too, but it will not output result to position list or error list. 
								\item :vimgrep and :lvimgrep difference lies in write to position list or error list.
								\item :lv can follow \% to represent the current file
								\item $\backslash$vv is just syntax sugar of :vimgrep. You don't need input word manually, and it will open quickfix window directly. The shortcomting is you can't edit regex manually. 
								\item You can use :GrepOptions to turn on recursive, so vv will find all .h or .c or .cpp files.
								\item For :lv, you can use **/*.cpp to search all .cpp files recursively. You also can input complex regex manually, So :lv is more flexible than vv. vv is more convient than :lv. 

		\end{enumerate}
		
\item :marks :ju :reg are three useful commands. 		
\end{itemize}
...
\hline 
	...
	\begin{enumerate}
			\item Closing windows is different with deleting buffer.When you use :bd command. It will close the windows too. If you want to just close buffer. You shold use :bn to switch to another buffer, then use :ls to know the buffe number which you want to close, in the end. use :bd num close to close it background without close current windows.
			...
			\item If you want to open a buffer, you can use :sp | b buffernum. It's very convinent. 
	\end{enumerate}
	\end{itemize}
...
\begin{itemize}
		...
		\item Ctrl-n,p will navigate all the items in the pop-up menu. And Ctrl-e close pop-up menu, and Ctrl-y accept the current item. 
...
\begin{itemize}
		...
		\item \textbf{f command is very very usefule!}
		\item If you want to jump to long distance, <leader><leader>f is your best friend. 
		\item You also can try <leader><leader>n,N command, it will 
\end{itemize}
...
\begin{itemize}
		...
		\item For practical usage, you still need to come back to look at its tutorial. 
...
\subsection{Basic}
\begin{itemize}
	\item For some large scale C/C++ project, There are a lot of source code which are in a deep directory structure. You can use below tips:
\begin{enumerate}
		\item \linuxcommand{find . -name "*.c" | xargs git add} to add all source code to git, Then you can build a branch test. You can apply this methods to all .cpp and .hpp and .h files. 
				
		\item Any time you edit a source code, run \linuxcommand{:!ctags -R .} inside of vim. Then :set tags=tags. Then you can use ctrl+] to jump to defination. and ctrl+t to come back.

		\item Any time you eidt a source code, run \linuxcommand{:!cscope -Rbq}, then run :cs add cs<tab>. add cscope.out file. Then you can use cscope command to jump. Detail commands can be found in cscope section.
		...
		\item First, you need to use Nerdtree to get all directory information, Then set path+=<paste directory here>.In the end, you can use gf command to pen header file

		\item Use F8 to open tagbar windows, it shows all global varaible and function in this files.

		\item Then, use Ctrl+] to jump to funciton and global varaible defination, Another options is ctrl+\ g. The first is ctags method, and the second is cscope method. Pay attention, If there is only open option, it will jump there directly, you can use ctrl+t or ctrl+o jump back. If there are more options, both methods will open a new windows. 

		\item gD and $[$i have the same meaning, gD will jump to it, and $[$i just show this line below. They don't have semantic measning, just the first line in the current file. <C-]> is just like :tag, only support global scope. \textbf{If you want to jump to the definiation of local varialbe, 1) use \# or * 2)use :lv <C-r><C-w>\% to open a position list window, it will list all word in the current file. 3) use Easygrep plugin}

		\item ctrl+\ d and c will show all calling functions(c) and called function(d). It's command in cscope

		\item ctrl+\ s and ctrl+\ t are different, result of t will be greater than s, It just think it as text, even it's in the comment. But s will only search source code. It also include context information, such as fucntion context. \textbf{You need to use s command more.}

		\item For local varialbe, you can use * and \# comand to search it. After that, you can press F4 to toggle hisearch. $[$i will show the first position of current cursor word in this file. For some common varaible, such as i,j, It's not right. so \# method is better.

		\item If you want to search withing specifc directory, you can use easyGrep tool. Detail can be seen esayGrep section.
\end{enumerate}
...
\begin{itemize}
		...
		\item All kinds of configuration play import role in EasyGrep. You can use :let g:EasyGrepRoot to check configuratin options value. It doesn't need to follow value.
		...
		\item If you want to toggle recursive, you can run :GrepOptions and then press letter r, it's much easier than remember :let g:EasyGrepRecursive name. 
