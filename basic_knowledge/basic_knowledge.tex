...
Another popular option is VMWare. It will support copy/paste and windows size will be changed properly. The newest version is vmware workstation pro 17 and it is also free. After you install linux, you also need to run below commands to support copy/paste and adjustable windows. 
...
\begin{enumerate}
	...
	\item In one word, if you want to use docker + wsl2 combination, it's good to make hyper-v, but vmware maybe doesn't work well. If you disable hyper-v, vmware works well, but docker+wsl2 has something wrong.  That is all.
...
\subsection{Shell type}
	There are two basic shells, one is bash and the other is tcsh. They can be changed by calling \linuxcommand{bash} or \linuxcommand{tcsh}.  You can use \linuxcommand{echo \$0} to see which shell you are using right now. By now, a new shell is zsh, It support more features. 
...
\subsection{Terminal tips and key shortcut}
	...
	You can add your frequently used working directories to the CDPATH environment variable. When the cd command cannot find the specified file or directory in the current directory, it will automatically search in the directories listed in CDPATH.
...
\section{Net}
	...
	The ip command is a unified networking tool for Linux systems. The ip command helps view and configure routing, interfaces, network devices, and tunnels. The command is part of the iproute2 package and replaces many older networking tools, such as the route, ifconfig and netstat commands. 
...
\section{User}
		...
		sudo visudo, then In the bottom of the file, add the following line:
\begin{lstlisting}
...
\section{Processes}
	...
	Each command-line terminal corresponds to a shell process, and any program you run within that terminal is actually a child process spawned by the shell. Normally, the shell process will block (wait) until the child process exits before accepting new input.

	When you add an \& at the end of a command, it simply tells the shell not to block — it can immediately return and accept new commands. However, no matter what, if you close the terminal, all the child processes tied to that shell will be terminated as well.

	When you run a command over SSH, it is started as a child process of your shell session. When you disconnect, the shell exits, and all child processes receive a SIGHUP signal (hang-up), causing them to terminate. If you run a command like this:\linuxcommand{(cmd \&)}. The cmd process is adopted by the systemd init system (or another parent like init), instead of staying tied to your shell. This means that closing the terminal has no effect on the command — it continues to run in the background, independently. \linuxcommand{nohup some\_cmd \& } The nohup command works based on a similar principle.
...
shift+select is copy,
right mouse is paste, 

in vim, if you want to paste, you need to config->General->MobaXterm Keyboard shortcut->Paste in terminal -> change it to Shift+esc if you keyboard has no insert key.  

\subsection{tmux}
	A key idea about tmux is session and windows can be attached and detached.


	\includegraphics[width=0.7\linewidth]{pics/tmux}
...
\item Resize pane: Ctrl-b + arrow keys
...
both .test should have the same archtecture. remote gdb has two advantages 1) local has all source code and git repo support,  2) remote has all necessary hardware configuration. You only need to compile in local and then copy binary to server(test dvice)
\begin{lstlisting}
