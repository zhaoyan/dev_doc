...
\section{Linux basic}
\begin{itemize}
		\item You can install linux in virtual box. That is very good virtual machine. After you install it. you need to install guest addition, then restart the computer. 
		
		\item You can also specify the share folder between host and guest. you need to run \linuxcommand{sudo groupadd usbfs} and \linuxcommand{sudo groupadd vboxsf}. Then use \linuxcommand{cat /etc/group} to check if you add successfully. Then sudo adduser yan vboxusers, sudo adduser yan vboxsf. In this way, you can use shared folder between host and guest. 
