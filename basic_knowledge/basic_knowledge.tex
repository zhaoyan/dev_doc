...
\section{Linux system}
	...
	\par There is a good book on deep Linux knowledge called "How Linux Works." Another useful book is "The Linux Command Line."

	\par There is a good article to introduce Linux distribution. The name is "RedHat vs Debian : Administrative Point of View". You can google and read it. In one word. RedHat is stable, commercial server with small amount of packages. Fedora and CentOS are based on it. Fedora is cutting edge implementation(For test). Another big group is Debian, It's also stable with much more packages. Ubuntu is based on Debian. Debian-based distributions tend to be community-driven, open and freedom-oriented, and highly general-purpose.

	\par Ubuntu is mostly used for desktop, and Mint is a sleek and quick version of Ubuntu, CentOS is for server, It's more stable. If you use virtual machine, recommend Mint, if you want to install linux directly on a computer, you can use Ubuntu system, because it's more friend toward beginner.

	
	\par There are two projects you can use to customize your own Linux: OpenWrt and Yocto. In addition to general-purpose Linux, there are specialized distributions designed for embedded and custom hardware. OpenWrt is a Linux distribution tailored for routers and network appliances; it is lightweight, modular, and optimized for devices with limited memory and storage.


	\par Yocto is a flexible build system for creating custom Linux distributions. It is not a distribution itself but a collection of tools and metadata that lets developers define exactly which packages, libraries, and configuration options are included in a target system.

	\par There are two Interface Framework or toolkits(widgets), Qt and GTK.  KDE is based on QT, and Gnome and Cinnamon(Mint) is based on GTK. Both Gnome and KDE are desktop environment. it includes window manager (Mutter), panels / UI, settings and apps (files, terminal, etc.) desktop environment is different with Display manager.  

	\par Instead, the most common way to start an X server is with a display manager, a program that starts the server and puts a login box on the screen. When you log in, the display manager starts a set of clients, such as a window manager and file manager, so that you can start to use the machine. Many different display managers are available, such as gdm (for GNOME) and kdm (for KDE). The lightdm in the argument list for the preceding X server invocation is a cross-platform display manager meant to be able to start GNOME or KDE sessions.

\begin{enumerate}
\item Boot
\item Display Manager (GDM)  ← login screen
\item User logs in
\item GNOME session starts
\item Mutter runs (window manager/compositor)
\end{enumerate}

	\par GUI toolkits (like GTK and Qt) provide the widgets and APIs developers use to build applications, but a complete desktop environment needs more than just toolkits. It also includes support files such as icons, fonts, window decorations, and configuration files that define a theme and visual style. Documentation and design guidelines tie these pieces together, specifying conventions for menus, window titles, keyboard shortcuts, and how applications should respond to system events so the user experience feels consistent across apps.
	
	\par When developing windowed applications on Linux, developers usually do not write X11 or Wayland protocol code directly; instead, they use GUI frameworks such as GTK or Qt.
	
	\par On a typical Ubuntu desktop, the operating system Ubuntu provides the platform on which the GNOME desktop runs; GNOME uses Mutter as its compositor, which implements the Wayland protocol to handle window display and input. Applications are usually built with GUI toolkits such as GTK (used heavily by GNOME apps) or Qt, and these toolkits render UI elements (buttons, windows, layouts) and communicate with the underlying system through Wayland; Mutter then composes all application windows into the final image shown on the screen.
\begin{lstlisting}
	App (Qt app)        App (GTK app)
	      |                   |
	      v                   v
	     Qt                 GTK
	        |             |
...
\end{lstlisting}	
	...
	\par You can use \linuxcommand{uname} to check basic information about your computer, detail can be seen uname --help. \linuxcommand{cat /etc/os-release} or \linuxcommand{lsb\_release -a} tell you distro version.

	\par The processor type (or name) indicates the architecture for which the processor was designed. The hardware machine name must be compatible with the processor type; in other words, they must match. Finally, the hardware platform describes the instruction set the hardware uses, and it does not need to be a higher version than the processor type. i686 = x86\_64. \linuxcommand{lscpu} will tell you how many cores you have. Socket means a physical CPU, and "cores per socket" is the number of cores in that CPU.

	\par Ctrl+Alt+F1\ldots F6 switch to the text consoles. Ctrl+Alt+F7 returns to the GUI. If F7 doesn't work, try F8.

	\par General application details, such as a distribution’s defaults for a user interface, don’t belong in /etc. System default configuration files not meant to be customized also are usually found elsewhere, as with the prepackaged systemd unit files in /usr/lib/systemd.
...
You can install Linux in \textbf{Virtual Box} which is very good virtual machine. After you install it, you need to install guest addition, then restart the computer. If you just want to try different version linux, a better way is to use docker, it's lightweight than virtual box. 

	\par You can also specify the share folder between host and guest. You need to run \linuxcommand{sudo groupadd usbfs} and \linuxcommand{sudo groupadd vboxsf}. Then use \linuxcommand{cat /etc/group} to check if you add successfully. Then \linuxcommand{sudo adduser yan vboxusers,} \linuxcommand{sudo adduser yan vboxsf}. In this way, you can use shared folder between host and guest. 

	\par After you install linux, you also need to run below commands to support copy/paste and adjustable windows. 

\begin{lstlisting}[language=bash]
vmware-toolbox-cmd -v #to see which VMware you are running. if not
sudo apt install open-vm-tools
sudo apt install open-vm-tools-desktop	
systemctl status open-vm-tools //check if it running
\end{lstlisting}

	\par Another popular option is VMWare. It will support copy/paste and windows size will be changed properly. The newest version is vmware workstation pro 17 and it is also free. Most of time, open-vm-tools has been running when you use VMWare. 

	\par Wsl+docker or VMWare? 

\begin{enumerate}
	\item Hyper-V Mode ("Windows Hypervisor Platform") is introduced due to Windows updates (especially from Windows 10 onward). Uses Microsoft's Hyper-V as the underlying hypervisor.  Needed when Hyper-V is enabled (e.g., for WSL2, Windows Sandbox, Device Guard).

	\item VMware Native Mode ("VMware Virtualization Engine"). Traditional VMware virtualization. Uses VMware's own hypervisor directly. Offers better performance, feature completeness, and stability. Works only when Hyper-V is disabled on the host.

	\item In one word, if you want to use docker + wsl2 combination, it's good to make hyper-v, but vmware maybe doesn't work well. If you disable hyper-v, vmware works well, but docker+wsl2 has something wrong.  VMware 16+ can work with wsl at the same time. 
	
	\item Before you install vmware, disable hyper-V and memory integrity. Detail can be googled.
\end{enumerate}

	\par Python’s virtual environment feature creates a version of Python with only the packages that you want. The way to start is by creating a new directory for the environment like this: \linuxcommand{python3 -m venv test-venv}. 
	
	\par In summary, there are fourth levels: The first is vmware, The second is docker or LXC, the third is runtime-base virtualization such as python venv and nvm in node, The last option is load mode. 
...
\subsection{User}
	...
	\par \linuxcommand{cat /etc/passwd} will tell you all the users and which shell they are using.  

	\par Don't log in root, you can use \linuxcommand{sudo} follow your command

	\par \linuxcommand{whoami} tell you account information.
			
	\par sudo visudo, then In the bottom of the file, add the following line:
\begin{lstlisting}
$USER ALL=(ALL) NOPASSWD: ALL
\end{lstlisting}		
		it will disable sudo passwd
		
		\par run passwd user as the superuser. Use adduser and userdel to add and remove users, respectively. don't edite /etc/passwd directly. If yo have to edit text, you can use vipw programme. 
		
		\par In fact, you’ll rarely even use them, because most users now log in either through a graphical interface such as gdm or remotely with SSH, neither of which uses getty or login.
	
\section{Init}

 The basic Linux startup flow is:
 \begin{enumerate}
 \item The machine’s BIOS or boot firmware loads and runs a boot loader.
 
 \item The boot loader finds the kernel image on disk, loads it into memory, and starts it.
 
 \item The kernel initializes the devices and its drivers.
 
 \item The kernel mounts the root filesystem.
 
 \item The kernel starts a program called init with a process ID of 1. This point is the user space start.
 
 \item init sets the rest of the system processes in motion.
 
 \item At some point, init starts a process allowing you to log in, usually at the end or near the end of the boot sequence.
 \end{enumerate}
 
 \par Then, If you start with GUI:
 \begin{enumerate}
     \item Display Manager shows login (Ubuntu GUI)
     \item Launch non-interactive login shell
     \item Run \texttt{\~/.profile} (set PATH, etc.)
     \item Initialize desktop environment
     \item You can start terminal emulate here.
     \item Start non-login interactive shell
     \item Run \texttt{\~/.bashrc}
 \end{enumerate}
 
 
	\par use \linuxcommand{journalctl -k} to displays the messages from the current boot, but you can view previous boots with the -b option.
 
\subsection{bootloader}

	Before the kernel and init start, a boot loader program loads and starts the kernel. Although its job sounds simple—loading the kernel into memory from disk and starting it with a set of kernel parameters—it is actually more complex than it appears. This is because the boot loader must handle several important questions and decisions during the process.

	\par GRUB A near-universal standard on Linux systems, with BIOS/MBR version and UEFI versions. To access the GRUB menu, press and hold SHIFT when your BIOS startup screen first appears, or ESC if your system has UEFI.

	\par /boot is important directory. The GRUB configuration directory is usually /boot/grub or /boot/grub2. We won’t modify /boot/grub/grub.cfg directly
\begin{enumerate}
\item sudo vim /etc/default/grub -->GRUB\_CMDLINE\_LINUX="quiet splash"
\item sudo update-grub  --> generate grub.cfg. 
\end{enumerate}


\begin{enumerate}
\item edit /etc/default/grub
\item sudo update-grub, it is ubuntu's wrapper grub-mkconfig
\end{enumerate}

\begin{tabular}{|l|l|l|}
\hline
\textbf{Feature} & \textbf{BIOS} & \textbf{UEFI} \\
\hline
Boot method & raw disk sectors & files \\
\hline
Location & MBR & EFI partition \\
\hline
Filesystem aware & No & Yes \\
\hline
Bootloader type & binary blob & .efi executable \\
\hline
Flexibility & low & high \\
\hline
\end{tabular}

	\par /boot/efi must be a separate mount because it is a FAT32 partition required by UEFI firmware. Your Linux root (/) is usually ext4, xfs, or btrfs, and UEFI cannot read those filesystems.
\begin{lstlisting}
/dev/sda1 -> /boot/efi   (FAT32, EFI System Partition)
/dev/sda2 -> /           (ext4, root filesystem)
\end{lstlisting}

	\par GRUB does not need / to be mounted; it can read the disk directly using its own filesystem drivers, load the kernel and initramfs, and only then the kernel mounts the root filesystem (/).

	\par Most modern Linux distributions include a bootloader (often GRUB), but not all systems use GRUB. There are some alternative bootloaders, such as U-Boot, used widely in embedded devices. /boot/uEnv.txt is used by U-Boot in embedded systems to configure boot behavior.

	\par /proc/cmdline is not used by GRUB; it is created by the kernel to expose the boot parameters that GRUB (or another bootloader) passed during startup.

\subsection{Kernel}

When the Linux kernel starts, it receives a set of text-based kernel parameters containing a few additional system details. The parameters specify many different types of behavior, such as the amount of diagnostic output the kernel should produce and device driver-specific options. You can view the parameters passed to your system’s currently running kernel by looking at the \linuxcommand{/proc/cmdline} file.

	\par A user session is the environment created when a user logs in, including all processes, permissions, and resources tied to that login. It uses logind to track sessions, and \linuxcommand{loginctl} can show all the user sessions. Logging in from two different places creates two separate user sessions, even with the same username.

	\par /init comes from initramfs. After the root filesystem is ready, initramfs switches root and runs /sbin/init.


\subsection{device tree}
	The udev system enables user-space programs to automatically configure and use new devices.

	\par These device files, often called device nodes, allow programs to interact with hardware using standard file operations. While they are commonly used by programmers, some devices can also be accessed with standard utilities such as cat, meaning that even non-programmers can interact with certain devices directly.
\begin{lstlisting}
echo 'yan' > /dev/null  # /dev/null is a device.
\end{lstlisting}
...
Kernel starts
   v
initramfs runs   <- temporary
   v
mount rootfs     <- real system
   v
switch_root
   v
real /sbin/init runs
\end{lstlisting}
	...
	\par If the file type character is b, c, p, or s, the file represents a device. These letters stand for block device, character device, pipe, and socket, respectively.

	\par To provide a uniform view of attached devices based on their hardware attributes, the Linux kernel offers the sysfs interface, which organizes information through a hierarchy of files and directories. The base path for device-related information is /sys/devices.

	\par The /dev directory allows user processes to interact with devices, while the /sys/devices path is used to view information about devices and manage their attributes.

\begin{lstlisting}
udevadm info --query=all --name=/dev/sda
\end{lstlisting}

	\par The program dd is extremely useful when you are working with block and character devices. Its sole function is to read from an input file or stream and write to an output file or stream, possibly doing some encoding conversion on the way.

\begin{lstlisting}
dd if=/dev/zero of=new_file bs=1024 count=1
\end{lstlisting}

	\par Run cat /proc/devices to see the block and character devices for which your system currently has drivers. Each line consists of a number and name.

	\par Common devices name:
\begin{lstlisting}
Hard Disks: /dev/sd*
...
\end{lstlisting}
	...
	\par On any reasonably recent Linux system, you do not create your own device files; they’re created by devtmpfs and udev. However, it is instructive to see how to do so, and on a rare occasion, you might need to create a named pipe or a socket file.
\begin{enumerate}
\item hardware plugged in
\item kernel detects device
\item kernel sends uevent (netlink)
\item udev receives event
\item creates /dev node + applies rules
\end{enumerate}

 \par The devtmpfs filesystem was developed in response to the problem of device availability during boot. This filesystem is similar to the older devfs support, but simplified. The kernel creates device files as necessary, but it also notifies udevd that a new device is available. Upon receiving this signal, udevd does not create the device files, but it does perform device initialization along with setting permissions and notifying other processes that new devices are available. Additionally, it creates a number of symbolic links in /dev to further identify devices. You can find examples in the directory /dev/disk/by-id, where each attached disk has one or more entries.

\begin{enumerate}
\item kernel launch
\item mount devtmpfs (basic/dev)   
\item launch udevd   
\item udev use /sys + event refines /dev, including to apply some rules.
\item system continue launch
\end{enumerate}
...
/etc/udev/rules.d/
/lib/udev/rules.d/
SUBSYSTEM=="usb", ATTR{idVendor}=="1234", NAME="my_usb"
\end{lstlisting}

	\par udev does NOT manage hardware - the kernel does. udev only creates a user-friendly interface in /dev

	\par The udevadm program is an administration tool for udevd. You can reload udevd rules and trigger events, but perhaps the most powerful features of udevadm are the ability to search for and explore system devices and the ability to monitor uevents as udevd receives them from the kernel. The command syntax can be somewhat complicated, though. There are long and short forms for most options; we’ll use the long ones here.

\begin{lstlisting}
udevadm monitor
\end{lstlisting}

	\par Device Tree = hardware description; Kernel uses it to load/bind drivers.  Especially for embedded systems, no hardware discovery, so manual hardware description

	\par a device tree is A data structure (usually .dts/.dtb) that describes hardware on embedded systems
\begin{lstlisting}
...
\end{lstlisting}
	...
	\par driver code to match the device tree.
\begin{lstlisting}
.of_match_table = {
    { .compatible = "arm,pl011" },
}
\end{lstlisting}
	...
	\par A .ko file is a Linux kernel module — a piece of code that can be loaded into the kernel at runtime.
\begin{lstlisting}
/lib/modules/$(uname -r)/  Where .ko files are stored
...
\subsection{Basic}
	...
	\par dd and wodim is different command.  
\begin{lstlisting}
dd if=/dev/cdrom of=ubuntu.iso #write to iso from cd
wodim dev=/dev/cdrw image.iso # write iso back to cd
\end{lstlisting} 

	\par Common device name are: /dev/fd*(soft disk), /dev/hd*(IDE PATA), and /dev/sd*(SCSI disk), /dev/sr*(optical drivers) /dev/lp* (printers.)

	\par unmount is important because it can save some memory usage. with each device, system will allocate buffer inside the memory. So unmount a device when you don't use it. 
	
	\par \linuxcommand{du -h --max-depth=1} list current sub directory size. 

	\par A hard link points to the file by \textbf{inode}.  A symbolic link points to the file by \textbf{filename}. 
 
	\par There is no "real" hard link name; All hard links are equally valid names for the file. You can use \linuxcommand{ls -l}. The first number after the file mode is the link count(this count is represent hard link number).  For symbolic link, It just point to a filename, If origin file name changed, symbolic link will not be valid. symbolic link is very flexible,  It can be linked to a dir or it can be linked to different file system. But hard link has many restriction.  
		
	\par \linuxcommand{find -L / -samefile path/to/foo.txt} find all files links to foo.txt

	\par Absolute directory must begin with root directory /.
		
	\par Linux doesn't use extension name to specify file type, you should use \linuxcommand{file fileName} to judge it.  

	\par When you use ls -l, the first character stands for different kinds: -:file, d:Dir, l:link file, b:interface of device. So you can use \linuxcommand{ls -al | grep \^{}d} to show all the directories. 
	 
	\par \linuxcommand{ls -d */} will list only directory without all files in it. if you want to see all files in it. omit -d. \linuxcommand{*/} tells shell to expands to all the directory, only * will expands all directorys and files. 
 
	
         
	\par The Dir Structure:
\begin{center}
...
\begin{tabular}{|l|l|l|}
\hline
\textbf{Command} & \textbf{What it shows} & \textbf{Level} \\ \hline
lsblk & Block devices \& partitions & Hardware view \\ \hline
fdisk & Partition table & Disk layout \\ \hline
mkfs & make filesytem &  \\ \hline
mout & mount to a directory &  \\ \hline
df & Mounted filesystem usage & Filesystem view \\ \hline
free & Memory (RAM) usage & Memory \\ \hline
\end{tabular}
\caption{Linux system information commands}
\end{table}


\begin{lstlisting}
blockdev --getsize64 /dev/mmcblk0p43 
\end{lstlisting}
	 

	\includegraphics[scale=0.8]{pics/basic_file_system_clip}

	\par A good tool software in linux system is GParted, which  is used for creating, deleting, resizing, moving, checking, and copying disk partitions and their file systems.  You also can use df command to check your system \textbf{partitions}. You can create 1) partitions by GParted 2) mkdir a dir 3) then mount them by adding below text in /etc/fstab file. \verb=/dev/sda3 /home/yan/llvm ext4 defaults 0 0=
   
	\par \textbf{/etc, /bin, /dev, /lib and /sbin can NOT be put in different partition with root /. }  


	\par The method of mounting filesystems discussed in the preceding section depends on device names. However, device names can change because they depend on the order in which the kernel finds the devices. To solve this problem, you can identify and mount filesystems by their universally unique identifier (UUID), an industry standard for unique “serial numbers” to identify objects in a computer system. Filesystem creation programs like mke2fs generate a UUID when initializing the filesystem data structure. To view a list of devices and the corresponding filesystems and UUIDs on your system, use the blkid (block ID) program:

	\par To mount filesystems at boot time and take the drudgery out of the mount command, Linux systems keep a permanent list of filesystems and options in /etc/fstab. This is a plaintext file in a very simple format.
	
	\par some special file sytem is proc(/porc), sysfs (/sys), tmpfs(/run), squashfs(A type of read-only filesystem where content is stored in a compressed format and extracted on demand through a loopback device. One example use is in the snap package management system that mounts packages under the /snap directory.) and overlay (used in virtualization)
	
	\par you can absolutely run mkfs directly on a flash drive without using fdisk to creat a partition table first.
	
	\par Both mount and fsck operate on a filesystem, not on partition or device directly.
	
	\par usually, you should build partition for swap, you can build a partition for /, but a better way is to use different partition for /var, /home, because these directory write very often.  


 
\subsection{Network file}

	\par Samba run in linux, then windows can access its file and printer service. SSHFS run in linux, then ohter linux can access.
\begin{lstlisting}
sshfs username@host:dir mountpoint
\end{lstlisting}

	\par go to the directory containing the file(s) and run:
\begin{lstlisting}
python -m SimpleHTTPServer
\end{lstlisting}
		 This starts a basic web server that makes the current directory available to any browser on the network. then you can copy file from there. 		 
		 
	\par To get rsync working between two hosts, you must install the rsync program on both the source and destination, and you’ll need a way to access one machine from the other. The -n option tells rsync to operate in “dry run” mode.
\begin{lstlisting}
$ rsync -av file.txt user@192.168.1.100:/home/user/
$ rsync -a dir/ host:dest_dir #doesn't build dir in destination.
$ rsync -a dir host:dest_dir  #build dir
\end{lstlisting}

	\par rsync is a very power command with many options, such as checking transfer. Whenever you need to copy numerous files, rsync should be one of the first utilities that comes to mind. Running rsync in batch mode is particularly useful for copying the same set of files to multiple hosts, because it speeds up long transfers and makes it possible to resume when interrupted.
	
	\par SSHFS Mount a remote directory over SSH (secure, simple). Network File System(NFS) is A dedicated network file system (fast, designed for performance)
\begin{lstlisting}
sudo apt install nfs-kernel-server  #server
mount -t nfs 192.168.1.10:/remote/path /mnt  #client
\end{lstlisting}	

	\par To understand VFS and FUSE conception. When you write a file by SSHFS, below steps are follow. 
\begin{enumerate}
\item User program write() syscall
\item :contentReference[oaicite:1]{index=1} (VFS)
\item FUSE kernel module
\item FUSE daemon (sshfs process in user space)
\item SFTP (over SSH connection)
\item Remote server (sshd -> sftp subsystem)
\item Remote filesystem
\end{enumerate}

	\par All cloud storages are nearly all implemented as FUSE (File System in User Space) interfaces. because a FUSE handler is nothing more than a user-space daemon that acts as an intermediary between the data source and the kernel.
	
	\par SMB is a protocol, CIFS is an old version of SMB and Samba is a software implementation of SMB. SMB is native for windows, and linux use Samba to implement it. CIFS still used as a name in Linux, but actually uses SMB2/SMB3 internally. 
\begin{lstlisting}
\\linux-server\shared 
#based on Samba, act like a windows file server, used by windows client.

mount -t cifs //server/share /mnt # use of cifs, 
\end{lstlisting}


\subsection{Permission or file mode}

	\par umask controls the default permissions of new files and directories. When a program creates: a file -> starts from 666; a directory -> starts from 777. Then the system applies: final\_permission = default - umask. It’s actually bit masking (removing permissions).


	\par \linuxcommand{ls -al} will list all the files ownership and permission. Basically, all the Linux system support \linuxcommand{ll} command directly, so you can use it directly. 
			  
	\par There are three different groups: Owner, Group, and Other. In each group,  there are three different permissions: r, w, x;
...
\subsection{Commands about File}
	...
	\linuxcommand{du} means disk usage. \linuxcommand{du -h} command displays the sizes in kilobytes of all files in the specified directory.  df command displays the amount of unused space left in your disk system. \linuxcommand{du -sh *} will show your every directory size. \linuxcommand{du -sh * | sort -h} will sort all directory size. Don't use sort -n here, it will put 1.1G ahead 1.2M. 
...
\begin{tabular}{|l|c|c|}
	\hline
	\textbf{Feature} & \textbf{\texttt{cp}} & \textbf{\texttt{rsync}} \\
	\hline Basic copy & Yes & Yes \\
	\hline Preserve metadata & Yes (with \texttt{-a}) & Yes \\
	\hline Efficient sync & No & Yes (copies only diffs) \\
	\hline Network support & No & Yes (via SSH) \\
	\hline Resume partial copies & No & Yes \\
	\hline Progress display & No & Yes (\texttt{--progress}) \\
	\hline Compression support & No & Yes (\texttt{-z}) \\
	\hline Delete extra files & No & Yes (\texttt{--delete}) \\
	\hline
\end{tabular}
...
\subsubsection{Find command}
	\linuxcommand{find} command follows a path directly. A common usage is \linuxcommand{find . -iname "yan"}

	\linuxcommand{find . -name *.txt} and \linuxcommand{find . "*.txt"} are different. In the first example, shell will receive *.a and expand it to a.txt b.txt c.txt.... In the second example, find command receive *.a.  So the first find command, if in your current directory, there is a.txt. find will not look all *.txt recursively. The key points is: if you don't add double quotes, the shell will interpret *.txt first. If you add double quotes, shell will not interpret(expand) it and give it to find command directly. In the previous example, you also can use \linuxcommand{find . $\backslash$*.txt}. Use $\backslash$ to stop shell to expand it.  

	\linuxcommand{find . -name "ab"} will not find "abc" file. You need to use "ab*" to get it. 
...
\subsubsection{Grep command}
	...
	\linuxcommand{grep -r -w -i "match" file1 file2}.\textbf{Put files name in the end. That is basic pattern. If it's a directory, add -r after grep. }Such as, \linuxcommand{grep -r 'word' .} -n will give match line number. Once you know line number, you can vim +num file to open it.  -v inverse result. -l will suppress your output. It will only output file name. -l will help you to deal with file with some pipe command, such as copy or rm files with some match words in them.  
	
	A, B, C use to print line around match. \linuxcommand{grep -B 2 -A 1 'computer' file} -c will only print  match number, -C3 will print context 3 line information .  
	
	\linuxcommand{grep --include=$\backslash$*.\{c,h\} -rnw  -e "pattern" /path} -w stands match the whole word. -l (letter L) can be added to have just the file name. This will only search through the files which have .c or .h extensions. Similarly a sample use is --exclude. --include usage will recursive search all the sub directory. You can use *.txt, but it only works on current directory.

	export GREP\_OPTIONS='--color=always' will give grep command color output.

	\linuxcommand{echo --include=*}, just like previous example, it will fail to look for filename begin with --incl.. So it will print out No match. If you use \linuxcommand{echo --include=$\backslash$*}, it will print literal string out. \linuxcommand{echo --inclue=$\backslash$*.\{a,b\}} will output --include=*.a and --include=*.b
	 	
	From all previous examples, if you use \verb!--include="*.{c,h}"!. It will prevent shell to expand * and big bracket and is not right. For new version grep, You don't need double quote. --include will prevent shell expand. But for old grep, it doesn't work. I recommend to use \verb!--include=\*.{h,c}! to escape * symbol. It work on both new and old version.  
...
\subsubsection{Regex}
	grep support regex, You need to use -E option to support extended regex. \linuxcommand{grep -E 'abc\{1,2\}d' file}.  That's very power feature. Here, I just list a few basic usage. Since you usually type regular expressions within shell commands, it is good practice to enclose the regular expression in single quotes (') to stop the shell from expanding it. 
...
\begin{enumerate}
	...
	\begin{tabular}{|l|l|}
		...
		\hline
		\hline .        &  Any character except newline \\
		\hline $\backslash$d       & Digit (0–9) \\
		\hline $\backslash$D       & Non-digit \\
		\hline $\backslash$w       & Word character (alphanumeric or underscore) \\
		\hline $\backslash$W       & Non-word character \\
		\hline $\backslash$s       & Whitespace (space, tab, newline) \\
		\hline $\backslash$S       & Non-whitespace \\
		\hline \texttt{[abc]}    & Any one of a, b, or c \\
		\hline \texttt{[\textasciicircum abc] }   & Any character except a, b, or c \\
		\hline \texttt{[a-z]}    & Any lowercase letter from a to z \\
		\hline
	\end{tabular}
	...
	\begin{tabular}{|l|l|}
		...
		\textbf{Syntax} & \textbf{Meaning} \\
		\hline
		\hline *        & 0 or more times \\
		\hline +        & 1 or more times \\
		\hline ?        & 0 or 1 time \\
		\hline \{n\}     & Exactly n times \\
		\hline \{n,\}    & At least n times \\
		\hline \{n,m\}   & Between n and m times \\
		\hline
	\end{tabular}
	
	\item Anchors: $\backslash$b matches a word boundary, i.e., the position between a word character ([A-Za-z0-9\_]) and a non-word character (like space or punctuation).
\begin{lstlisting}
My cat chased a caterpillar 
\bcat\b match the word cat, but not caterpillar
\end{lstlisting}
	\begin{tabular}{|l|l|}
		\hline
		\textbf{Syntax} & \textbf{Meaning} \\
		\hline
		\hline \textasciicircum       & Start of line \\
		\hline \$       & End of line \\
		\hline $\backslash$b       & Word boundary \\
		\hline $\backslash$B       & Not a word boundary \\
		\hline $\backslash$A       & Start of string \\
		\hline $\backslash$Z       & End of string \\
		\hline
	\end{tabular}
	
	\item grous. () and (?). By placing part of a regular expression inside round brackets or parentheses, you can group that part of the regular expression together. This allows you to apply a \textbf{quantifier} to the entire group or to restrict \textbf{alternation} to part of the regex.  Only parentheses can be used for grouping. Square brackets define a character class, and curly braces are used by a quantifier with specific limits.	
\end{enumerate}


\section{Shell}

\subsection{Shell basic}
	...
	\par When you run a command in shell, it will fork, exec and wait, When you understand fork and exec, then it understands background job, (no wait), and know parent and child relationship. 
	
	\par Run ls will create a new child process, run a.sh creates a new child subshell, because in the beginning , there is \#!/bin/bash. When you understand it, you will know difference between \linuxcommand{source a.sh} and ./a.sh. You can run bash inside shell, then run exit to exit child shell. ./a.sh just like /bin/bash a.sh
	
	\par Another way to use subshell
\begin{lstlisting}
bash uglyprogram
( unglyprogram )  
\end{lstlisting}

	\par \linuxcommand{bash a.sh} and \linuxcommand{./a.sh} run the script in a new process, do NOT affect your current shell environment export, cd, variables won’t persist. For \linuxcommand{bash a.sh}, You explicitly tell Bash to interpret the file. For \linuxcommand{./a.sh}, it uses the shebang line inside the script. 
	
	\par \linuxcommand{env bash} will run first bash found in your \$PATH
	
	\par Before running the command, the shell looks for variables, globs, and other substitutions and performs the substitutions if they appear. The shell passes the results of the substitutions to the command. The avoid this problem, use single quotes. 
\begin{lstlisting}
grep a.*b file  <= shell expand .*
grep 'a.*b' file <= shell give .* to grep command directly. 
\end{lstlisting}

	\par Double quotes (") work just like single quotes, except that the shell expands any variables that appear within double quotes.
\begin{lstlisting}
echo "$100" # only print 00, becuase $1 is varaible. 
\end{lstlisting}

	\par The backslash and quote must appear outside any pair of single quotes.
\begin{lstlisting}
echo 'this isn'\''t a forward slash: \' # \' must outside of a pair of '', 
...
\hline
\end{tabular}
\caption{Common Shell Variables}
\end{table}	
	
	\par When we speak of the command line, we are really referring to the shell. hash mark (\#) rather than a dollar sign, the terminal session has superuser privileges. This means either we are logged in as the root user or we selected a terminal emulator that provides superuser (administrative) privileges.
	
	\par There are two basic shells, one is bash and the other is tcsh. They can be changed by calling \linuxcommand{bash} or \linuxcommand{tcsh}.  You can use \linuxcommand{echo \$0} to see which shell you are using right now. By now, a new shell is zsh, It support more features. 

	\par You can use \linuxcommand{chsh}, -l will list all available shell, and -s follow new shell full path name, such as \linuxcommand{chsh -s /bin/bash}.

	\par Every time when you open a new terminal, It will read .bashrc or .rshrc. It depends on what shell you are using, that is why you need to use \linuxcommand{echo \$0} to know which shell you are using. It will use export(bash) or setenv(csh) to load all envoriment variable. 

	\par In tcsh, you can use bindkey -e and bindkey -v to change to emacs mode and vi mode.In bash, you can use set -o vi or set -o emacs.
	
	\par Remember what shell scripts do best: manipulate simple files and commands. As stated earlier, if you find yourself writing something that looks convoluted, especially if it involves complicated string or arithmetic operations, don’t be afraid to look to a scripting language like Python. 
	

\subsection{Wild character and quote}

	The \linuxcommand{set -x} command enables command tracing (or command echoing) in the shell. With tracing turned on, you can observe exactly what commands the shell is executing.
...
\begin{center}
	...
	\begin{tabular}{|p{0.1\textwidth}|p{0.3\textwidth}|p{0.08\textwidth}|p{0.3\textwidth}|}
		\hline
		\textbf{Pattern} & \textbf{Description} & \textbf{Example} & \textbf{Effect} \\
		\hline
		\texttt{!!} & Previous command & \texttt{!!} & Repeats the last command \\
		\hline
		\texttt{!n} & Command number \texttt{n} from history & \texttt{!42} & Runs the 42nd command \\
		\hline
		\texttt{!-n} & \texttt{n}-th command before now & \texttt{!-2} & Runs the command 2 steps back \\
		\hline
		\texttt{!string} & Most recent command starting with \texttt{string} & \texttt{!git} & Runs last command starting with \texttt{git} \\
		\hline
		\texttt{!?string?} & Most recent command \textbf{containing} \texttt{string} & \texttt{!?npm?} & Runs last command containing \texttt{npm} \\
		\hline
		\texttt{\^old\^new} & Substitution in previous command & \texttt{\^foo\^bar} & Replace \texttt{foo} with \texttt{bar} in previous command \\
		\hline
	\end{tabular}

\end{center}

The special command !\$ will be replaced with the last argument of the previous command. The special command !* will be replaced with all the arguments (typically file paths) from the previous command.
...
echo !*    # expands to: file1.txt file2.txt
\end{lstlisting}
	...
	\begin{enumerate}
		\item * and ? and [ ] all expands according to file name. For example \linuxcommand{echo [a-d]c}. When shell see [ ], it will expand according to file names. If it doesn't match, it will print No match. If there are bc file or dc file. [a-d]c will be expanded to "bc dc".
		
		\item a* will match abc and a.txt, a? only match two characters file name and the first character is a. 
		
		\item \{\} will \textbf{NOT} match file name, a\{.h,.c\} just expands to \linuxcommand{a.h a.c}, and there is a space between a.h and a.c. Even there is no a.h and a.c in the \textbf{Don't use space inside \{\} }.
		
		\item When you use \linuxcommand{echo \{*.txt,*.c\}}. It expands to \linuxcommand{*.txt *.c}. Then shell try to interpret * to a.txt b.c if there is a.txt and b.c in your directory.
	\end{enumerate}

\begin{tabular}{|l|l|l|}
\hline
\textbf{Usage} & \textbf{Meaning} & \textbf{Who interprets} \\
\hline
\texttt{\$\{var\}} & variable expansion & shell \\
\hline
\texttt{\{ cmd; \}} & command grouping & shell \\
\hline
\texttt{\{1..5\}} & brace expansion & shell \\
\hline
\texttt{\{\}} (alone) & literal text & nobody \\
\hline
\texttt{find ... \{\}} & filename placeholder & find command \\
\hline
\end{tabular}

	If you don't want shell to interpret wild character, use $\backslash$
\begin{lstlisting}
echo a\* # just print out a*. 
echo 'a*' or "a*", both output a*.
\end{lstlisting}

	A good example of wild character is \linuxcommand{grep --exclude=a$\backslash$*.h 'a*b' *.h}. You should know three * meaning in previous command. Let me explain below: 
\begin{enumerate}
	\item The first * is to avoid shell to interpret it. But in the new version linux, you don't need $\backslash$ any more; 
	
	\item The second one use single quotes(or double quotes)avoid shell interpret, it just means a*b file literally. (* is valid character in linux.).
	
	\item For the third one, shell interprets it.  
\end{enumerate}
	
	
\begin{lstlisting}[language=bash]
yan.zhao@MB-XTPX4XQG9G test % find . -name *.c
./a.c  #shell expand(match) *.c to a.c first. then give it to find command

yan.zhao@MB-XTPX4XQG9G test % find . -name "*.c"
./a.c
./linux/a.c
./linux/b.c  #shell give "*.c" directly to find, find interpret *.c inside it.	
\end{lstlisting}

		
\subsection{Environment variable}

	\par Three main variable
\begin{lstlisting}
printenv | less
set | less #include some bash script funcitons
alias 
\end{lstlisting}

	\par Some interesting variable
\begin{table}[h]
\centering
...
\hline
PS1 & Stands for “prompt string 1.” This defines the contents of the shell prompt. It can be extensively customized. \\
\hline
PWD & The current working directory. \\
\hline
TERM & The name of your terminal type. Unix-like systems support many terminal protocols; this variable sets the protocol used with your terminal emulator. \\
\hline
TZ & Specifies your time zone. Most Unix-like systems maintain the internal clock in UTC and display local time using this variable. \\
\hline
USER & Your username. \\
\hline
\end{tabular}
...
\end{lstlisting}
	...
	\item For interactive non-login shell, know difference between source app , ./app and bash app. bash app force app to run in bash, to ignore the shebang \#!/bin/sh
\end{itemize}

	\par For tcsh, read .cshrc file, for zsh, read .zshrc file. This goes pretty far back in the Unix history. rc stands for "run commands", and makes sense actually.

	\par echo \$0 will show "bash", it's not login shell. If it shows "-bash", it is login shell.  
	...
	\par Why we need variable, just like macro in C language. With variable, we can configure and customize an application outside. And we can customize a variable to affect a lot of applications. Such as \$PATH.
				
	\par For different shell, you have different syntax in shell script. For example, for bash shell you can use YanVar=123, There is no space between =. You also can use unset command "unset YanVar". Or use check command: \linuxcommand{echo \$YanVar}. Different shell have different syntax, tcsh muse use set. Pay attention, in shell, there is no variable, just macro, so expansion is key idea to understand it's behavior. Such as expansion order.
	
	Three methods to use \$
\begin{lstlisting}[language=bash]
today=$(date) # preferred over legacy backticks `...`
echo ${today}_file #today is variable, without {} bring ambiguity.
...
\end{lstlisting}
	...
	\linuxcommand{setenv} is csh command. In bash, you can use \linuxcommand{export} directly. csh doesn't use =, bashrc doesn't use backslash.
\begin{lstlisting}
#in .cshrc
...
\end{lstlisting}
	...
	If There are a few environment variables need to be set, You'd better put them into a script file. But you have to use \linuxcommand{source a.sh} to make sure a.sh running in the current process, not a child process. 

	Two shells don't share history and environment variables. To fix history problem, always use exit command in terminal windows. When a shell starts, it loads history from the file (e.g., ~/.bash\_history) into memory. When a shell exits, it writes its in-memory history back to the file. So if you open two terminals: Each has its own copy of the history in memory. They do not see each other's commands immediately. They will overwrite each other's history when they exit — unless managed carefully.
	
	Based on above introduction, try best to use only one terminal to keep history intact. 
	
	
\subsection{Pipe}

	Standard input refers to functions in programming languages such as scanf. In contrast, arguments refer to the character array args passed into a program's main function. As previously mentioned in Linux file descriptors, the pipe (|) and redirection (<, >) operators pass data to a program's standard input. Without pipe and redirection operator, the string will be used as arguments, such as \linuxcommand{mkdir} and \linuxcommand{cd} command
	
	\par The \linuxcommand{rm} command in its source code is designed \textbf{NOT} to read from standard input, but instead to receive command-line arguments specifying which files to delete. In contrast, the cat command accepts both standard input and command-line arguments. vim, nano and passwd only accept standar input.

	\par pipe command can be called "filter", It will accept input from STDIN, perform operations, then send it to STDOUT. Such as \textbf{grep, uniq, sort, fmt, pr, head, cut, tee, join, paste, expand, split, tr, awk, sed, less}

\begin{center}
	\includegraphics[width=0.4\linewidth]{pics/pipe}
\end{center}
		
	\par For example, \linuxcommand{cat file | mkdir} won't work — mkdir doesn’t take input from stdin. You can't judge if a command is filter by running it. "cat" will hang up to wait for you from STDIN, but less will show error message. A easy way is to use \linuxcommand{echo "Yan" | command}. If it's work, then command support pipe.

	
	\par Sometimes, a command need a file name as input or output, but you don't want to give a filename, It usually used in pipe commands, such as \linuxcommand{gzip -dc a.tar.gz  | tar -xvf  - }. It just like \linuxcommand{gzip -dc a.tar.gz  | tar -xvf  /dev/stdin}. "-" is common on many commands that want to use a file, such as tar and gzip. It allows you to specify standard input or output rather than an actual file name. For example, if you run \linuxcommand{gzip --help | grep standard} will show "With no FILE, or when FILE is -, read standard input."
...
\end{lstlisting}
	...
	\item In above command, if too many files are found, it may cause an “Argument list too long” error. If a filename contains spaces or special characters, it may be misinterpreted as multiple arguments, which could lead to accidental deletion.
	
\item xargs calls rm in batches, making it more efficient and able to avoid the "argument list too long" problem. Also avoid spaces or special character problem in file name
\begin{lstlisting}
	find . -name "*.log" -print0 | xargs -0 rm
\end{lstlisting}	
	
	
	\item The best solution is to use -exec in find command
\begin{lstlisting}
...
\subsubsection{Basic}
	...
	The first line of script is \verb=#!/bin/bash=.  This is a special clue(shebang) given to the shell indicating what program is used to interpret the script. Other scripting languages such as perl, awk, tcl, Tk, and python can also use this mechanism.
...
\subsubsection{Variable} 
	...
	no space aournd = when you define a variable.
\begin{lstlisting}
count=12
...
\end{lstlisting}
	...
	\par print the contains of variable (HOME) and not the HOME. You must use \$ followed by variable name to print variable. \verb= echo $HOME= print the variable contents. And \verb=echo HOME= just print "HOME" string. It's a little confused for a C programmer, but you need to be used to it. 
...
\begin{tabular}{|l|l|}
\hline
\textbf{Clipboard} & \textbf{How to use} \\
\hline
Primary   & select + middle click \\
\hline
Clipboard & Ctrl+Shift+C / V      \\
\hline
\end{tabular}	
	
	In modern ubuntu, we use wl\_copy and wl\_paste. -p will use primary(highlight text) -n will not include newline.  

	\par \linuxcommand{!!} run the previous command, \linuxcommand{!\$} is the previous argument. \linuxcommand{\$?} is the last bash command result, If it's 0, means that everything is OK. Detail can be found in google "Become a Command Line Ninja With These Time-Saving Shortcuts''
	
	\par You can add your frequently used working directories to the CDPATH environment variable. When the cd command cannot find the specified file or directory in the current directory, it will automatically search in the directories listed in CDPATH.

	\par You can use \linuxcommand{xdg-open file} to use default app to open the file. Such as \linuxcommand{xdg-open a.pdf}. 


	\par Motion shortcut keys are list below: Ctrl+p or up down arrow key can go to the previous command, this is very useful.  Ctrl+a, k will delete very long command line. alt+f, b jump with word. Another useful is Ctrl+r. Ctrl+L is clear the screen. 
	
	\par !number Repeat history list item number.
...
\par Delete shortcut keys: You can use "hw" to delete previous character or word. "dd" use to delete forward character or word, "uk" to sentence beginning and end. In Vim, the keyboard shortcut is not the same. \textbf{They are bash edit key shortcut.}
...
\subsection{Time}
	...
	\par For Linux file time: there are three time stamps: atime (access time), it is when the file was last read.  ctime is the inode change time, while mtime is the file modification time.  mtime changes when you write to the file. It is the age of the data in the file. \textbf{Whenever atime or mtime changes, so does ctime, except you use touch command} But ctime changes a few extra times. For example, it will change if you change the owner or the permissions on the file.  

	\par timestamp will be used in many linux commands, ls -l will show modification time. and you can use \linuxcommand{stat fileName} to see all the three time. The can be used in find command. 

	\par atime sometimes will not updated by visiting a file. Atime updates are by far the biggest IO performance deficiency that Linux has today. So sometimes it's disable when mount in option in /etc/fstab

	\par \linuxcommand{touch} can change time of a file or you can use it to produce an empty file.  \linuxcommand{touch -a existFile} change access time and ctime. \linuxcommand{touch -t existFile} change modify time and ctime. \linuxcommand{touch -c existFile} change a,c and m time. 
            
	\par \linuxcommand{touch -t YYMMDDHHmm} will set mtime and atime to the date you want and it sets ctime to \textbf{NOW}. You have complete control over mtime, but the system stays in control of ctime. So mtime is a little bit like the date on a letter while ctime is like the postmark on the envelope. System use ctime to do backup job. An example can be found in my evernote book mark. 
		
	
\section{Systemd unit}
	\par systemd is a modern init system and service manager for Linux, designed to replace traditional tools like: SysVinit (classic init scripts), init.d.
	
	\par /etc/inittab is system V init config. it uses scripts in /etc/init.d/. runlevel directories is in /etc/rc0.d and each contains symlins like s10network. Limitations of SysV init.
	\begin{itemize}
	    \item Sequential startup (slow)
	    \item Hardcoded order (S10, S20, \ldots)
	    \item No dependency awareness
	    \item Hard to manage complex systems
	\end{itemize}
	
	\par Why systemd replaced it
	\textbf{Modern systemd:}
	\begin{itemize}
	    \item Uses dependencies instead of numbers
	    \item Starts services in parallel
	    \item Replaces runlevels with targets
	\end{itemize}
	
	\par Some common systemd units: ssh.service cron.service networking.service docker.service.
	
	\par There are two main directories for systemd configuration: the system unit directory (global configuration; usually /lib/systemd/system or /usr/lib/systemd/system) and the system configuration directory (local definitions; usually /etc/systemd/system).
	
	\par To know all the unit files, use below two commands:
\begin{lstlisting}
systemctl list-units  #currently loaded/running units
systemctl list-unit-files #all installed unit files
\end{lstlisting}
	
	\par "enable" will affect boot behavior and "start" affect current runtime. enable needs instructions on where to hook the unit into the boot graph—and those instructions live in the [Install] section.
...
\end{lstlisting}
	...
	\par /lib/systemd/system/  Default unit files provided by packages (like cron, ssh, nginx, etc.) Installed automatically when you install software
	
	\par /etc/systemd/system/  Used for: Custom unit files (you create). Overrides of default service files. Safer to edit — these take priority over /lib/systemd/system/

	\par To prevent confusion, stick to this rule: avoid making changes to the system unit directory, because your distribution will maintain it for you. Make your local changes to the system configuration directory. This general rule also applies system wide. When given the choice between modifying something in /usr and /etc, always change /etc. You should normally put your own unit files in the system configuration directory (/etc/systemd/system) so that you won’t confuse them with anything that came with your distribution and so that the distribution won’t overwrite them when you upgrade.

	\par If your system has /usr/lib/systemd and /etc/systemd directories, you have systemd. If you have an /etc/inittab file, you’re probably running System V init. The systemd init is one of the newest init implementations on Linux. In addition to handling the regular boot process, systemd aims to incorporate the functionality of a number of standard Unix services, such as cron and inetd. It takes some inspiration from Apple’s launchd.

	\par However, if you’ve changed a unit configuration file, you can tell systemd to reload the file in one of two ways:
\begin{lstlisting}
systemctl reload unit  #Reloads just the configuration for unit.
systemctl daemon-reload #Reloads all unit configurations.
\end{lstlisting}

	\par if you change unit, use  daemon-reload. if you change config, such as nginx.conf, use reload, and it will not interrupt service.
\begin{lstlisting}
vim my.service
...
\end{lstlisting}
	...
	\par One way that systemd is more ambitious than previous versions of init is that it doesn’t just operate processes and services; it can also manage filesystem mounts, monitor network connection requests, run timers, and more. Each capability is called a unit type, and each specific function (such as a service) is called a unit. When you turn on a unit, you activate it. Each unit has its own configuration file;

\begin{table}[h]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Type} & \textbf{Suffix} & \textbf{Description} \\ \hline
service & .service & Manage and start services (most common) \\ \hline
mount & .mount & Mount filesystems \\ \hline
device & .device & Represent hardware devices \\ \hline
target & .target & Group units (similar to runlevels) \\ \hline
socket & .socket & Socket-based activation \\ \hline
timer & .timer & Scheduled tasks (replacement for cron) \\ \hline
\end{tabular}
\caption{systemd Unit Types}
\end{table}

	\par One unit is just a file.
\begin{lstlisting}
[Unit]
...
\end{lstlisting}
	...
	\par Open a terminal and create a new service file under /etc/systemd/system/, for example:  sudo vim /etc/systemd/system/myservice.service
\begin{lstlisting}
[Unit]
Description=My Service After Time Sync
After=network-online.target time-sync.target
Wants=network-online.target time-sync.target

[Service]
Type=simple
ExecStart=/usr/bin/echo "Time synced, service started"
Restart=always
...
\end{lstlisting}
	...
	\par If your unit file has an [Install] section, you need to “enable” the unit before activating it and disable it before remove it. 

	\par One of systemd’s features is the ability to delay a unit startup until it is absolutely needed. You need to define a resource unit R and relationship between you unit A.

	\par As systemd has grown in popularity, it has grown to include support for a few tasks not related to startup and service management, both directly and through auxiliary compatibility layers. You may notice the numerous programs in /lib/systemd; these are the executables related to those functions. such as udevd, journald and resolved. Their name usually start with systemd-
...
\subsection{Log}
	...
	\par dmesg Shows kernel ring buffer (kernel messages only). it direct from Linux kernel and includes mostly: drivers hardware boot messages. when you reboot, content will be lost. journald also include boot message.

\begin{lstlisting}
Kernel -> dmesg buffer -> journald -> journalctl
\end{lstlisting}
              
	\par \linuxcommand{cat /var/log/syslog} and \linuxcommand{sudo journalctl -r}  are two major log check commands.
	
\begin{enumerate}
	\item Your app sends log messages to the local syslog service. These messages end up in /var/log/syslog or the systemd journal.
...
\end{lstlisting}	
	...
	\item Using stdout/stderr (recommended with systemd) Write logs to standard output/error. If your app runs as a systemd service, systemd-journald automatically captures these. You can then view logs with journalctl -u yourapp.service.
\end{enumerate}

	\par journald only (modern, minimal).  journald + syslog (MOST COMMON), syslog only (old style) used in some old embedded system. Systems keep both systemd-journald and rsyslog because they solve different problems. syslog advantages is easy to grep and work with legacy tools.
	
\begin{table}[h]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Filter Type} & \textbf{Command Example} & \textbf{Description} \\
\hline
Boot & journalctl -b & Show logs from current boot \\
\hline
Previous Boot & journalctl -b -1 & Show logs from previous boot \\
\hline
Service (Unit) & journalctl -u ssh & Logs for a specific service \\
\hline
Time Range & journalctl --since "2024-01-01" & Logs since a given time \\
\hline
Time Range (until) & journalctl --until "10:00" & Logs up to a given time \\
\hline
Follow (live) & journalctl -f & Follow logs in real time \\
\hline
Priority & journalctl -p err & Filter by log level (err, warning, info) \\
\hline
Kernel Logs & journalctl -k & Show kernel messages only \\
\hline
User Logs & journalctl --user & Show user session logs \\
\hline
Executable & journalctl /usr/bin/ssh & Logs from a specific binary \\
\hline
PID & journalctl \_PID=1234 & Logs for a specific process ID \\
\hline
\end{tabular}
\caption{Common journalctl Filters}
\end{table}
...
\hline
System-level & vmstat / iostat / uptime \\
\hline
Process-level & top / pidstat \\
\hline
Historical data & atop \\
\hline
Timing tools & time \\
\hline
\end{tabular}
...
\begin{tabular}{|l|l|}
\hline
\textbf{Step} & \textbf{Tools} \\
\hline
Is the system healthy? & vmstat / uptime \\
\hline
What kind of issue is it? & vmstat / iostat \\
\hline
Which process is causing it? & top / pidstat \\
\hline
What happened in the past? & atop \\
\hline
Is it inside the program? & strace / time \\
\hline
\end{tabular}
...
\hline
System is slow / lagging & vmstat \\
\hline
High CPU usage & top \\
\hline
Slow disk & iostat \\
\hline
Find problematic process & top / pidstat \\
\hline
Check historical performance & atop \\
\hline
Program is slow & time \\
\hline
Network / system calls & strace \\
\hline
\end{tabular}
...
\subsection{resource}
	...
	\par A process typically has two attributes: RUID and EUID. They represent who you are and who you are acting as. In most cases, they are the same. Why is there a distinction? Because of setuid programs. For example, a program with permissions like rwxs (such as passwd): when you run it, the EUID is set to the file owner (usually root). This allows the program to perform privileged operations. The saved UID exists so that the program can temporarily drop privileges and later regain them when needed.

	\par understand session leader, process group(job); session is identified by SID and usually tied to one terminal (TTY) and created by setsid(). session leader is process which PID == SID. 
\begin{lstlisting}
Terminal (TTY)
   |
   v
Session (SID=1000)
   |
   |--> Shell (session leader) PID = 1000
   |
   |-->PGID 2000 (job %1, foreground)
   |     |--> process A
   |     |--> process B
   |
   |-->PGID 3000 (job %2, background)
         |--> process C
\end{lstlisting}

	\par Above session is different with session in tmux, session in tmux is user space conception. 

	\par to understand process group is important because only foreground job receive signal, such as SIGINT(ctrl+c) and SIGTSTP(ctrl+z).  pipeline = one process group. job is perspective from shell, not from kernel. 
	
\begin{lstlisting}
find . | grep foo # when ctrl+c, both find and grep will get SIGINT and terminate. 
\end{lstlisting}
	
	\par “stop” and “terminate” are fundamentally different, Stop (Ctrl+Z) process is paused (suspended), does not exit and stays in memory and state becomes T (stopped). 
	
	\par The kill command is not just "kill, it used to send signals to programs. SIGINT 2, SIGTERM 15, and SIGKILL 9 all “stop” a program, but in very different ways. Think of them as escalating levels of force. SIGINT -> user interruption (Ctrl+C) and SIGTERM -> polite request to exit and SIGKILL -> force kill (no negotiation). The same idea is also applied for STOP 19 and TSTP 20. 
	
	\par vmstat is a little old, but still used in embedded linux(Yocto system), you can also use pstree, xload and tload.

\subsubsection{IO}
	\par iostat shows the statistics for your machine’s current uptime:

	\par If you need to dig even deeper to see I/O resources used by individual processes, the iotop tool can help. Using iotop is similar to using top.

	\par However, this display refreshes over time, and each update
erases the previous output. The pidstat utility allows you to see the resource consumption of a process over time in the style of vmstat.

\subsubsection{CPU}
\subsubsection{memory}

	\par The CPU has a memory management unit (MMU) to add flexibility in memory access. The kernel assists the MMU by breaking down the memory used by processes into smaller chunks called pages. The kernel maintains a data structure, called a page table, that maps a process’s virtual page addresses to real page addresses in memory. As a process accesses memory, the MMU translates the virtual addresses used by the process into real addresses based on the kernel’s page table.
...
\end{lstlisting} 
	...
	\par use top or ps. When running top, use f to change the displayed fields and select nMaj as one of the columns to display the number of major page faults. Selecting vMj (the number of major page faults since the last update) can be helpful if you’re trying to track down a process that might be misbehaving. When using ps, you can use a custom output format to view the page faults for a particular process. Here’s an example for PID 20365:
\begin{lstlisting}
ps -o pid,min_flt,maj_flt 20365
\end{lstlisting}
	

	\par \linuxcommand{ps m -o pid,tid,command} will list both pid and thread id. 

	\par In fact, you probably won’t need to alter nice values much because many Linux systems have only a single user, and that user doesn’t perform much real computation. (The nice value was much more important back when there were many users on a single machine.)
	
	\par There are two ways to run programs on a repeating schedule: cron, and systemd timer units.

	\par If ldd doesn't installed in embedded device, use readelf on your host machine.
\begin{lstlisting}
readelf -d my_demo | grep NEEDED
...
\end{lstlisting}
	...
	\par Each command-line terminal corresponds to a shell process, and any program you run within that terminal is actually a child process spawned by the shell. Normally, the shell process will block (wait) until the child process exits before accepting new input.

	\par When you add an \& at the end of a command, it simply tells the shell not to block — it can immediately return and accept new commands. However, no matter what, if you close the terminal, all the child processes tied to that shell will be terminated as well.

	\par When you run a command over SSH, it is started as a child process of your shell session. When you disconnect, the shell exits, and all child processes receive a SIGHUP signal (hang-up), causing them to terminate. If you run a command like this:\linuxcommand{(cmd \&)}. The cmd process is adopted by the systemd init system (or another parent like init), instead of staying tied to your shell. This means that closing the terminal has no effect on the command — it continues to run in the background, independently. \linuxcommand{nohup some\_cmd \& } The nohup command works based on a similar principle.

	\par CTRL-C aborts the app, CTRL-Z \textbf{suspend} app and put it to background, you can use bg command to re-continue this job on background. CTRL-D is EOF. \textbf{C is cancel, D is end.}

	\par \linuxcommand{ps -l} and \linuxcommand{ps aux} are two common processes check command. ps aux will product a lot content, so we often use \linuxcommand{ps aux | grep user}. In status column, S is sleep, T is stop, and R is run. + symbol means it's in foregound. 

	\par \linuxcommand{./hello \&} will put hello to background. Another useful commands are \linuxcommand{fg, bg, job}. \textbf{They just belongs to this bash}. You can use ONE bash shell to do multi tasks. They are very helpful when you are login sever with ssh command. At this time, you only have one terminal window. So multi-task is import for you. If you work on local workstation. You can open another terminal windows easily, These commands are not very helpful for you.
 
	\par View all the background jobs using jobs command,  If you have multiple background ground jobs, and would want to bring a certain job to the foreground,  \linuxcommand{fg \%2} will bring the job\#2 and \linuxcommand{kill \%2} will kill it.  
 
	\par Ctrl-Z will suspend process. If you want to continue, you need to 1) use jobs to know its number, 2) use bg \%num to restart it in background. 3)If you want to bring it back to foreground, use fg \%num. 
	...
	\linuxcommand{kill -s singal PIDnumber} will send signal to processes with PIDnumber. This command has some confusion with its name.
...
\par Think of File Groups as "Who can touch this?" and cgroups as "How much of the computer can this use?"

cat /proc/cgroups to see all the controllers.
...
\hline
Full tree (easy) & \texttt{systemd-cgls} \\
\hline
All cgroups (v2) & \texttt{find /sys/fs/cgroup} \\
\hline
All cgroups (v1) & \texttt{find /sys/fs/cgroup/<controller>} \\
\hline
Controllers list & \texttt{cat /proc/cgroups} \\
\hline
\end{tabular}
...
basis. For example, if you want to limit the amount of memory that a set of processes may cumulatively consume, a cgroup can do this.

\begin{lstlisting}
mkdir /sys/fs/cgroup/cpu/cg1
echo 100000 > /sys/fs/cgroup/cpu/cg1/cpu.cfs_period_us 
echo 50000  > /sys/fs/cgroup/cpu/cg1/cpu.cfs_quota_us
#In each 100 ms period, the task can run for 50 ms 50% CPU
echo <PIDof my_exe> > /sys/fs/cgroup/cpu/cg1/tasks
\end{lstlisting}
...
\subsubsection{Basic}
	...
	\par The ip command is a unified networking tool for Linux systems. The ip command helps view and configure routing, interfaces, network devices, and tunnels. The command is part of the iproute2 package and replaces many older networking tools, such as the route, ifconfig and netstat commands. 
	
	\par You can google "where is my IP address" to get you external IP, or use \linuxcommand{ipconfig} to know your internal IP. \linuxcommand{host} can know IP or host name from each other. Another Interesting tool is \linuxcommand{netstat} can tell you what connections are there in your computer. and \linuxcommand{traceroute} can trace the path in connection. They are some useful Internet connection tool.
	
		\par The file /etc/resolv.conf in Linux is used to configure DNS (Domain Name System) resolution — that is, how your system translates domain names like example.com into IP addresses. When ping 8.8.8.8 is fast, but ping google.com is slow, then DNS server has problem. 
	
		\par The file /etc/wpa\_supplicant/wpa\_supplicant.conf is the main configuration file for wpa\_supplicant, the tool responsible for managing Wi-Fi connections on many Linux systems, especially in headless or minimal environments (e.g. Raspberry Pi, embedded systems, or servers without NetworkManager).
	\begin{lstlisting}
	sudo systemctl status wpa\_supplicant.service
	sudo systemctl enable wpa\_supplicant.service
	\end{lstlisting}
	
		\par The enable command is used to enable the wpa\_supplicant service to start automatically at boot time. What it does: When you enable a service, it creates symbolic links in the system’s startup directories, so the service is started automatically when your system boots.
	
		\par \linuxcommand{sudo ip route del default via 100.87.195.237}
	
		\par The rfkill command in Linux is used to enable or disable wireless devices (such as Wi-Fi, Bluetooth, etc.) on your system. It's typically used to control the hardware state of network interfaces without having to interact with the system's GUI or network management tools.
	...
	\end{lstlisting}
		...
		\par \linuxcommand{sudo ip route show} show what route you are using to connect. 
	\begin{lstlisting}
	...
	\end{lstlisting}
		...
		\par The file /etc/dhcpcd.conf is the configuration file for the dhcpcd (DHCP Client Daemon) in Linux. This daemon is responsible for obtaining network configuration (like IP address, DNS, gateway, etc.) from a DHCP server on the network.
	
		\par By editing this file, you can configure network interfaces, set static IP addresses, and manage other DHCP-related settings.
	

	\par tun is level three virtual NIC, used in VPN, tap is level two virtual NIC, used in virtual machine.

\par Wi-Fi and Ethernet have MAC addresses. but cellular has IMEI and IMSI.

\par  know the fixed classes conceptions and common private subnetwork.
10.0.0.0 – 10.255.255.255 10.0.0.0/8
172.16.0.0 – 172.31.255.255 172.16.0.0/12  
192.168.0.0 – 192.168.255.255 192.168.0.0/16  65536

\par You can identify a connection by using the pair of IP addresses and port numbers. To view the connections currently open on your machine, use netstat.

\par How do you know if a port is well known? There’s no single way to tell, but a good place to start is to look in /etc/services, which translates well-known port numbers into names.


\par These commands use rfkill to control wireless devices like Wi-Fi and Bluetooth.
\begin{lstlisting}
sudo rfkill list  #Shows the status of all wireless (radio) devices
sudo rfkill unblock wifi #unblock turns Wi-Fi back on if it was disabled in software.
\end{lstlisting}

\par In Linux, the ARP cache is in the kernel. To view your machine’s ARP cache, use the ip neigh command.
\begin{lstlisting}
ip -4 neigh
...
\end{lstlisting}
	...
	\par for wi-fi, You can view and change kernel space device and network configuration
...
\end{lstlisting}
	...
	\par Some wireless connection work flow
\begin{lstlisting}
rfkill unblock wifi
...
\end{lstlisting}
	...
	\par For most wireless security setups, Linux relies on the wpa\_supplicant daemon to manage both authentication and encryption for a wireless network interface. This daemon can handle the WPA2 and WPA3. When the daemon first starts, it reads a configuration file (by default, /etc/wpa\_supplicant.conf

	\par Most embedded device doesn't use NetworkManger, but manually setup internet connection. 

	\par If you need more flexibility in connecting to a remote host than a command like telnet host port allows, use netcat (or nc). netcat can connect to remote TCP/UDP ports, specify a local port, listen on ports, scan ports, redirect standard I/O to and from network connections, and more.


\subsubsection{IP command}

	\par The ip command is the current standard network configuration tool. (older command is ifconfig, for example in some old and embedded system, such as openwrt). The ip command’s output includes many details from the internet layer(s) and the physical layer. (Sometimes it doesn’t even include an internet address at all!). it will show the ip address and subnetwork, that information is very important. Pay attention to the CIDR notation. 
\begin{lstlisting}
#three important ip commands. 
ip address show/add/del
ip route show/add/del
ip link #list all network interface 
\end{lstlisting}

	\par how to add and delete route rule.
\begin{lstlisting}
...
\end{lstlisting}
above modification is temporary, edit /etc/network/interface to make it permanent. 

\subsubsection{Networkmanager}
\par The most widely used option on desktops and notebooks is NetworkManager. There is an add-on to systemd, called systemd-networkd. One for desktop and the other is for servers and embedded systems. NetworkManger is applet on the desktop and an icon in the upper right. nmcli command is shell command for networkmanger

\par One final detail of NetworkManager configuration relates to specifying additional system actions for when a network interface goes up or down. When a system’s network interface status changes, NetworkManager runs everything in /etc/NetworkManager/dispatcher.d with an argument such as up or down.

\par NetworkManager is feature-rich, dynamic, user-focused; it mainly used on desktop and laptop. systemd-networkd is minimal, deterministic, config-driven. most of time is used on servers and embedded/IoT device. The last resort is just use manually config at all. 
\begin{lstlisting}
systemctl status NetworkManager
systemctl status systemd-networkd
\end{lstlisting}

\par Some typical systemd-
\begin{enumerate}
\item systemd-udevd        -> detects network device

\item systemd-networkd     -> configures IP / routes

\item systemd-resolved     -> DNS resolution

\item systemd-timesyncd    -> time sync (TLS, logs, etc.)
\end{enumerate}


\subsubsection{DNS}
\par use host to find the IP adress behind a domain name. DNS look up follow below steps:
\begin{enumerate}
\item function in the shared library runs, it acts according to a set of rules (found in /etc/nsswitch.conf) to determine a plan of action on lookups. For example, the rules usually say that even before going to DNS, check for a manual override in the /etc/hosts file.

\item When the function decides to use DNS for the name lookup, it consults an additional configuration file to find a DNS name server. The name server is given as an IP address. The traditional configuration file for DNS servers is /etc/resolv.conf.
\end{enumerate} 

\par many machines (and routers, if acting as name servers) run an intermediate daemon to intercept name server requests and cache the reply, and then use the cached answers if possible. The most common of these daemons is systemd-resolved; you might also see dnsmasq or nscd on your system. Take a closer
look, though. If you’re running systemd-resolved, you might notice that resolv.conf isn’t even a file in /etc; it’s a link to an automatically generated file in /run. 

\par Linux name resolution (via systemd-resolved) is much more flexible than the old “just use one DNS server” model

\par You can check the current DNS settings with the resolvectl status command. note that this might be called systemd-resolve on older systems. resolvectl is essentially the modern replacement for systemd-resolve.

\par systemd-resolved acts as a manager: It can:
\begin{enumerate}
\item use DNS for internet domains 
\item use mDNS for .local
\item use LLMNR for local fallback
\end{enumerate}
and choose per interface


\par The /etc/nsswitch.conf file is the traditional interface for controlling several name-related precedence settings on your system, such as user and password information, and it has a host lookup setting. When a program asks: "Find the IP for example.com" The system needs to decide:

\begin{enumerate}
\item check local file?
\item ask DNS?
\item use mDNS? 
\end{enumerate}
/etc/nsswitch.conf defines that order.


\subsubsection{dhcp}

	\par DHCP does not directly run ip route add, but it provides routing info, and the DHCP client installs the routes. DHCP server usually run in a router, not switcher.

	\par  You can have hook at dhcp side /lib/dhcpcd/dhcpcd-hooks/99-custom-route. It’s executed automatically at specific points in the DHCP lifecycle—so you don’t run it manually. You use it when you want to customize routing behavior after DHCP events.
\begin{lstlisting}
interface comes up
DHCP lease is acquired (BOUND)
lease is renewed (RENEW)
lease is lost (EXPIRE/STOP)

You need custom routing after DHCP assigns IP
DHCP-provided routes are not enough or incorrect
You want dynamic behavior tied to network changes
\end{lstlisting}
...
\end{lstlisting}
	...
	\par below is dhcpcd -U output
\begin{lstlisting}
sudo dhcpcd -U eth0 #Show DHCP lease information for interface eth0
sudo dhcpcd -k eth0 #Kill (stop) DHCP client on eth0
sudo dhcpcd eth0 #restart 
\end{lstlisting}

	\par Client config file is here: /etc/dhcpcd.conf.
\begin{lstlisting}
...
# Ensure eth0 does not get a default route
#nogateway <----------comment
\end{lstlisting}


	\par the Linux kernel does not automatically move packets from one private subnet to another. To enable this basic routing function, you need to enable IP forwarding in the router’s kernel with this command:
\begin{lstlisting}
sysctl -w net.ipv4.ip_forward=1
\end{lstlisting}

	\par To make this change permanent upon reboot, you can add it to your /etc/sysctl.conf file. Depending on your distribution, you may have the option to put it into a file in /etc/sysctl.d so that distribution updates won’t overwrite your changes.

	\par two private network can forward packet by sysctl -w net.ipv4.ip\_forward=1, but private network to wan must use NAT, because Private IPs (from RFC 1918): are not routable on the Internet routers on the Internet will drop them.

	\par NAT is the most commonly used way to share a single IP address with a private network, and it’s nearly universal in home and small office networks. In Linux, the variant of NAT that most people use is known as IP masquerading.
...
# iptables -A FORWARD -i enp0s2 -o enp0s31f6 -j ACCEPT #you need to know iptables syntax.
\end{lstlisting}

\par You likely won’t ever need to manually enter these commands unless you’re developing your own software, especially with so much special-purpose router hardware available. However, a variety of virtualization software can set up NAT for use in networking for virtual machines and containers.

	\par In NAT, This is actually PAT (Port Address Translation): Many devices share one public IP by using different ports. In Linux: managed by kernel conntrack subsystem and sudo cat /proc/net/nf\_conntrack to view it. 


\subsubsection{firewall}
\par Firewall rules in a series known as a chain. A set of chains makes up a table. As a packet moves through the various parts of the Linux networking subsystem, the kernel applies the rules in certain chains to the packets. For example, a new packet arriving from the physical layer is classified by the kernel as “input,” so it activates rules in chains corresponding to input.


\par All of these data structures are maintained by the kernel. The whole system is called iptables, with an iptables user-space command to create and manipulate the rules. The DROP policy tells the kernel to discard the packet. To set the policy on a chain, use iptables -P
\begin{lstlisting}
iptables -L
iptables -P FORWARD DROP
...
\par There are two basic kinds of firewall scenarios: one for protecting individual machines (where you set rules in each machine’s INPUT chain) and one for protecting a network of machines (where you set rules in the router’s FORWARD chain).


\subsubsection{Common commands}
\begin{lstlisting}
wget  -> download files (simple, automated)
curl  -> transfer data (flexible, API / scripting)
ftp   -> interactive file transfer protocol/tool
\end{lstlisting}


\textbf{Basic ip command}
\begin{lstlisting}
...
\textbf{Connection \& Reachability Testing}
\begin{lstlisting}
Ping
ping 8.8.8.8
ping -c 3 google.com
...
\textbf{Ports, Sockets \& Connections}
\begin{lstlisting}
ss -tulnp  #Show listening ports
ss -an  #Show all connections

(old version: netstat -tulnp)
\end{lstlisting}


\textbf{Network Speed / NIC Info}
\begin{lstlisting}
NIC speed \& duplex
ethtool eth0
...
Quectel logs
journalctl -u quectel_gpio
journalctl -u ModemManager
\end{lstlisting}


\textbf{Packet Capture \& Debugging (ADVANCED)}
\begin{lstlisting}
Capture packets
sudo tcpdump -i eth0
...
\textbf{Firewall \& Routing (Advanced)}
\begin{lstlisting}
Firewall
iptables -L
nft list ruleset
...
sudo   systemctl    restart   quectel_gpio.service

sudo journalctl -f -u quectel_gpio.service  //check journal
...
\begin{lstlisting}
below is sudo journalctl -f -u quectel_gpio.service, looks good result. 

requestRegistrationState2 MCC: 311, MNC: 480, PS: Attached, DataCap: LTE
Jan 08 22:17:15 ranger-e8fd quectel_gpio.sh[954]: [01-08_22:17:15:378] change mtu 1500 -> 1428
Jan 08 22:17:15 ranger-e8fd quectel_gpio.sh[954]: [01-08_22:17:15:378] ip link set dev wwan0 up
Jan 08 22:17:15 ranger-e8fd quectel_gpio.sh[954]: [01-08_22:17:15:476] No default.script found, it should be in '/usr/share/udhcpc/' or '/etc//udhcpc' depend on your udhcpc version!
Jan 08 22:17:15 ranger-e8fd quectel_gpio.sh[954]: [01-08_22:17:15:476] busybox udhcpc -f -n -q -t 5 -i wwan0
Jan 08 22:17:16 ranger-e8fd quectel_gpio.sh[998]: udhcpc: started, v1.30.1
Jan 08 22:17:16 ranger-e8fd quectel_gpio.sh[998]: udhcpc: sending discover
Jan 08 22:17:16 ranger-e8fd quectel_gpio.sh[998]: udhcpc: sending select for 100.91.208.77
Jan 08 22:17:16 ranger-e8fd quectel_gpio.sh[998]: udhcpc: lease of 100.91.208.77 obtained, lease time 7200
Jan 08 22:17:16 ranger-e8fd quectel_gpio.sh[954]: [01-08_22:17:16:161] ip -4 address flush dev wwan0
Jan 08 22:17:16 ranger-e8fd quectel_gpio.sh[954]: [01-08_22:17:16:167] ip -4 address add 100.91.208.77/30 dev wwan0
Jan 08 22:17:16 ranger-e8fd quectel_gpio.sh[954]: [01-08_22:17:16:174] ip -4 route add default via 100.91.208.78 dev wwan0
Jan 08 22:17:16 ranger-e8fd quectel_gpio.sh[1006]: RTNETLINK answers: File exists
Jan 08 22:17:16 ranger-e8fd quectel_gpio.sh[954]: [01-08_22:17:16:192] Warning: No route items found for wwan0
\end{lstlisting}
...
sudo killall wpa_supplicant
 ...
 sudo journalctl -u wpa_supplicant -n 100
 Dec 24 18:27:33 ranger-fe56 wpa_supplicant[473]: Successfully initialized wpa_supplicant
 Dec 24 18:27:33 ranger-fe56 wpa_supplicant[473]: Could not read interface wlan0 flags: No such device
 Dec 24 18:27:33 ranger-fe56 wpa_supplicant[473]: nl80211: Driver does not support authentication/association or connect commands
 Dec 24 18:27:33 ranger-fe56 wpa_supplicant[473]: nl80211: deinit ifname=wlan0 disabled_11b_rates=0
 Dec 24 18:27:33 ranger-fe56 wpa_supplicant[473]: Could not read interface wlan0 flags: No such device
 Dec 24 18:27:33 ranger-fe56 wpa_supplicant[473]: wlan0: Failed to initialize driver interface
 Dec 24 18:27:33 ranger-fe56 systemd[1]: wpa_supplicant.service: Main process exited, code=exited, status=255/EXCEPTION
 Dec 24 18:27:33 ranger-fe56 systemd[1]: wpa_supplicant.service: Failed with result 'exit-code'.
 Dec 24 18:27:34 ranger-fe56 systemd[1]: wpa_supplicant.service: Scheduled restart job, restart counter is at 4.
 Dec 24 18:27:34 ranger-fe56 systemd[1]: Stopped WPA supplicant.
 Dec 24 18:27:34 ranger-fe56 systemd[1]: Starting WPA supplicant...
 Dec 24 18:27:34 ranger-fe56 systemd[1]: Started WPA supplicant.
 Dec 24 18:27:34 ranger-fe56 wpa_supplicant[491]: Successfully initialized wpa_supplicant
 Dec 24 18:27:37 ranger-fe56 wpa_supplicant[491]: wlan0: Trying to associate with SSID 'Verizon_69ZSW4'
 Dec 24 18:27:40 ranger-fe56 wpa_supplicant[491]: wlan0: Associated with 98:c8:54:b2:57:cf
 Dec 24 18:27:40 ranger-fe56 wpa_supplicant[491]: wlan0: CTRL-EVENT-CONNECTED - Connection to 98:c8:54:b2:57:cf completed [id=0 id_str=]
 Dec 24 18:27:40 ranger-fe56 wpa_supplicant[491]: wlan0: CTRL-EVENT-SUBNET-STATUS-UPDATE status=0
...
cat /etc/resolv.conf
\begin{lstlisting}
search lan   <-- make these two item in the first three line.
nameserver 8.8.8.8
\end{lstlisting}
...
\subsection{VNC server}
 ...
 \par It can help you to get remote linux desktop, It's very helpful for a windows programmer to use GUI in the linux host.
 
 \par vncserver -help will list all the command optoins.
 
 \par You need to install VNC server in the linux with su priviliage, then run vncserver on the linux machine. Last on you windows machine, you can use putty and vncviewer to access the remote desk top. Detail can be googled, there are some reference pages available.

\subsection{clipboard}

	\par There are two kinds of clips: 1) Normal clip, Ctrl+C copy and Ctrl+V paste. 2)Xclip, shift+mouse left button select+copy. Mouse middle button to paste. Most linux system support both. But they are two different clipboards. If you use Ctrl+C command, You can't use mouse middle button to paste it.
			
	\par \linuxcommand{xclip -o} will output content in xclip. Not normal clip.
...
\subsection{Installing}
	...
	\par There are two kinds of installing method in linux, one is install from source code, The other is install binary package.

	\par By now, There are three main install methods from source file, one is make and the other is cmake, For cmake, you should use "Out of source" build. Another is use autotool, but this method is a little obsolete, don't study and use it any more. For some simple project, you can use make file directly. 

	\par If you install from source code, you need to download tarball, then read INSTALL or README(option), then run config to produce makefile, last run make and make install.
\begin{verbatim}
...
\end{verbatim}
     ...
     \par As root, you should put install a application under /usr/local. If you don't have su privilege, use \linuxcommand{--prefix}.  It will compile source first, when you make install.

	 \par You should not specify app name in prefix directory. Linux will put all execute binary to \$HOME/bin, and lib to \$HOME/lib or \$HOME/lib64. If It's development package, it will put header file to \$HOME/include, and document file in \$HOME/share. So just use \verb!--prefix=$HOME!.Then add \$HOME/bin to your path in .bashrc file.  
	
	 \par ./Configure use Makefile.in to produce Makefile. They are a set of automatic tools. You can see them in c++ web directory, but they are a little complicated, Kdevelop also use them.  Just know them.  Below is an example to install Perl.

	 \par If there is no configure execute in your directory, you can use autoreconf to generate it. Basically, The first thing to do is read INSTALL and README two documents. 

	 \par install some perl programme, If you want to install in your own directory, you can add PREFIX. That will assure you have permission on it
\begin{verbatim}
...
\end{verbatim}
	...
	\par "tee" command is used to store and view (both at the same time) the output of any other command. 

	\par Before you install package, you can use \linuxcommand{md5sum} or \linuxcommand{sha1sum} on the package to get finger print, then compare your finger print with official one on the website to check the files validation.  
				
	\par There are two main binary installing method RPM+YUM(online update) and dpkg+APT(apt-get). CentOS uses the first, and Ubuntu uses the second. Detail can be found in Vbird linux book.

	\par You can download the .deb files and use 'dpkg-deb -x' to extract them underneath your home directory. You will then have a lot of "fun" setting the PATH, LD\_LIBRARY\_PATH, and other variables. The more complex the program or app you're installing the more fun you'll be up for :) So this is your last resort if you don't have su permission.

	\par Recently, more and more programme has been put into github. Such as xclip, So you can first google some application name. Such as xclip, google it and found git address https://github.com/astrand/xclip. Then git clone gitaddress on you Download directory. After that, you can use configure and make....

	\par Some application has pre-compiled portable binary package, such as double commander. On it's homepage, you can find file  doublecmd-0.7.8.gtk2.x86\_64.tar.xz. You need to know two things, 1) You GUI is gtk or qt? 2) You computer is 32 or 64, then download right package for you.

\subsection{Other}

	\par BusyBox is a single binary that includes a wide range of essential Linux utilities, many of which are commonly used in embedded systems and environments with limited resources. It is often called "The Swiss Army Knife of Embedded Linux" because it provides a lightweight, compact, and highly customizable way to run a full range of Unix commands, all bundled into a single binary file.
	
	\par First, adjust system font, It will make all menu and window font bigger, Second, for specific application, such as terminal , double commander and chrome, You can change it's font by preference. 

	\par There is Dark reader extension for chrome browser. There are three applications: 1) terminal(vim)+solairized theme 2) double commander+solairized theme 3) Chrome browser+Darker reader. Most of time, They are quite enough for my development career. 

	\par To make less show Chinese, \linuxcommand{export LESS=-isMrf} I don't know what it means?

	\par Usually, virtual box think right Ctrl as default host key, it's not convenient in linux, because most of move command need right Ctrl, so you need change it.  In window, run VBoxManage.exe setextradata global GUI/Input/HostKey 165 can change it to right Alt. Here, I need to explian, the 165, it's virtual keycode defined by microsoft. you can find detail in google. Now I change it to Win\_L, value is 91.

	\par There are AltGr key to input multi-language character, but I don't need it by now, according to my laptop layout, I need to change it to Alt, so I can use move command shortcut. and define win\_menu to Ctrl. I finish it as follow: 
	\begin{enumerate}
		\item use \linuxcommand{xev} get keycode, AltGr is 108 and win\_menu is 135 
		\item create your own .Xmodmap and write keycode 108 = Alt\_L
		\item in .bashrc, add some statements:
\begin{lstlisting}
xmodmap -e "add Contrlo = Menu'' (this statement is very important)
xmodmap -e "keycode 133 = Control\_R''content...
\end{lstlisting}
...
\begin{lstlisting}
Your code is fine. The problem is Gunicorn timeout behavior with SSE (Server-Sent Events).

This line is the clue:

File "/usr/lib/python3.8/site-packages/gunicorn/workers/base.py", line 204, in handle_abort
    sys.exit(1)

Gunicorn killed the worker.

Why this happens

Your /events endpoint is:

while True:
    yield f"data: {counter}\n\n"
    time.sleep(1)

This request never finishes.

But Gunicorn sync worker expects requests to complete within timeout.
...
Gunicorn thinks worker is hung
sends abort signal
raises SystemExit
Fix 1 (simplest)
...
gunicorn app:app -w 1 -b 0.0.0.0:8000 --timeout 0

--timeout 0 means disable timeout.

This is usually correct for SSE.
...
Why sync worker is problematic for SSE

sync worker:

one request blocks one worker
SSE connections stay open forever
eventually all workers occupied
...
One browser SSE connection consumes entire server.
...
Also add SSE headers

Better Flask response:
...
You already partly did this, but ensure:
...
proxy_buffering off is VERY important for SSE.
...
SSE tech with python to show a good website. 

\section{JS and typescript}
“dynamically typed” and “weakly typed” are not the same thing. They describe different aspects of a language, and people often mix them up.
Dynamically typed → types are checked at runtime
Statically typed → types are checked at compile time

Strongly typed → strict, fewer implicit conversions
Weakly typed → allows more implicit conversions (sometimes surprising)

\begin{lstlisting}
"5" + 1   // "51" , weakly type, it can implicitly convert it. 
"5" - 1   // 4
\end{lstlisting}

\par TypeScript has five primitive types: strings, numbers, Booleans, undefined, and null. Everything else in the language is considered an object.

\par In typescript, void — “no useful return value” , void is mainly used for functions. null — an actual value, A variable explicitly holds “no value”

\par 

\par js use npm and python use pip, they are all package manager.

\par npm init will generate package.json
\par npm list express will check version, pay attention, it will not change package.json if major number is the same. 

\par One main export	use default export; Multiple exports use	named exports

\par JavaScript offers three different ways to declare a variable: var, let, and const.


\par JavaScript is single-threaded, which means that it can run only one task at a time.

\par tag and untagg string, arrow function, hoisted variables

\par promises and Asynchronous 
\begin{lstlisting}
...
//below is modern syntax with async/await syntax
//that is much like coroutine in C++.
...
\par spread operator is written as three dots (...).

\section{React and next.js}

\par A React element is: a lightweight JavaScript object that describes what the UI should look like

\par there are class component and function companent, they return elements. 

\par Custom hooks follow
a specific naming convention. They start with use, followed by an action
beginning with an uppercase letter. You should define only one functionality
per hook to make it easily testable.


\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Feature} & \textbf{Polling} & \textbf{SSE} & \textbf{WebSocket} \\
\hline
Direction & Client $\rightarrow$ Server & Server $\rightarrow$ Client & Both $\leftrightarrow$ \\
\hline
Connection & Many short & One long & One long \\
\hline
...
\begin{lstlisting}
every second: telemetry
large amount SSE clients
realtime dashboard
pub/sub
\end{lstlisting}
...
\section{ssh}
1) check ~/.ssh to see existing key
2) if not
...
4) copy public key to bitbucket website
 cat ~/.ssh/id_ed25519.pub 

5)ssh -T git@bitbucket.org
...
\subsection{codex}
There are a few points to use codex:
\begin{itemize}
	\item If you have a file, use the project option.
	\item One task per chat.
	\item For complex tasks, provide a plan first.
	\item Clearly define what "finished" means.
	\item Commit frequently to make reverting easier.
	\item On Windows, use the native agent; in WSL, install the Codex CLI.
	
	command+j -> terminal
	last message edit
	fork -->local or worktree
	agent.md
	steer
	
	
	when you have file, use project
	one task, one chat
	for complex, ask a plan first
	define what is finish
	more commit
	in agent, write what not do, or only change specific file. 
\end{itemize}
...
\par select is copy, right mouse is paste, 

\par If you open vim in mobaXterm, shift+select is copy, if you want to paste, you need to config->General->MobaXterm Keyboard shortcut->Paste in terminal -> change it to Shift+esc if you keyboard has no insert key.  

\par mobaXterm can remember password, that is very advantage feature. 


\subsection{tmux}
	\par tmux set -g mode-keys vi, it only affect copy and paste functions. 

	\par A key idea about tmux is session and windows can be attached and detached.
	
	\includegraphics[width=0.5\linewidth]{pics/tmux}


	In terminal windows, I run tmux, then close terminal windows, tmux will still be running even if you close the terminal window, as long as you didn't explicitly kill the session or exit all panes/windows inside tmux.
	...
	q or exit command are two common commands to close panel or quit the help. 
...
\begin{lstlisting}
tmux new -s <session-name> #0 is default if you don't name it
c+b d #detach 
tmux ls #list
tmux attach -t <session-name> #0 is default if you don't name it
tmux kill-session -t <session-name> #0 is default if you don't name it
\end{lstlisting}
...
Windows (like tabs)
\begin{itemize}
\item Create a new window: Ctrl-b c

\item List windows: Ctrl-b w

\item Switch window: Ctrl-b n (next), Ctrl-b p (previous), Ctrl-b <number>

\item Rename window: Ctrl-b ,

\item Close pane: exit or Ctrl-d

\end{itemize}

Panes (split panes)
\begin{itemize}
\item Split vertically: Ctrl-b \%

\item Split horizontally: Ctrl-b "

\item Switch between panes: Ctrl-b o

\item Resize pane: Ctrl-b + arrow keys\textbf{o}
\end{itemize}


Use windows when tasks are separate and you don’t need to see them all at once. Good for grouping different activities in the same session. Think of windows as separate workspaces, like browser tabs.

Examples:
\begin{itemize}
\item Window 1 → edit code (vim)

\item Window 2 → run server

\item Window 3 → monitor logs

\item Window 4 → run tests
\end{itemize}


Panes (splits inside a window) Use panes when tasks are related and you want to see multiple at the same time. Good for workflows where outputs must be visible together. Think of panes as split views inside a workspace.
Examples:
\begin{itemize}
\item Left pane → edit code

\item Right pane → run make or compiler

\item Bottom pane → tail logs

\item Top pane → run unit tests

\end{itemize}

Rule of Thumb:
Windows = high-level task separation (different contexts). Panes = low-level task coordination (same context, different tools).
...
\subsubsection{install on linux}
			...
			steps { 
				script{
					...
					usernameVariable: 'yan'
						...
						sshCommand remote: remote, sudo: false, command: "cd /home/yan && sudo docker run --user dev -v ./your/path:/home/dev/your/path nexus.app.com/repository/app/iot/u18owrt19:18.19 /bin/bash -c \"cd ~/your/path; python3 ../../utilities/scripts/compile_product219.py\" "
		...
		stage('Build old Pathfinder') {
			...
			steps { 
				...
				script{
					...
					docker.image('nexus.app.com/repository/app/iot/u18owrt19:18.19').inside('--user root') {
		...
		stage('Build new Pathfinder') {
			...
			steps { 
				...
				script{
					...
					docker.image('nexus.app.com/repository/app/iot/u22owrt22:22.22').inside('--user root') {
...
\subsection{Jenkin and sharepoint}
\begin{lstlisting}
pipeline {
    agent any
    environment {
        TENANT_ID = '867b575b-c37d-46ad-9c2b-2955a3d5e814'
        CLIENT_ID = '26475bf7-67e3-40b6-b5fa-0ba01dc52e63'
        CLIENT_SECRET = credentials('sp_secret')
        SHAREPOINT_SITE = 'https://appcorp.sharepoint.com/sites/IOTTeam'
        SHAREPOINT_FOLDER = '/Shared Documents/Documents/Software/Firmware'
    }
...
"https://graph.microsoft.com/v1.0/sites/app.sharepoint.com:/sites/IOTTeam" | python3 -c "import sys,json; print(json.load(sys.stdin)['id'])")
...
| python3 -c "import sys,json; print(json.load(sys.stdin)['value'][1]['id'])")
                ...
                # 2. Upload file to SharePoint via Microsoft Graph
                curl -X PUT \
                  -H "Authorization: Bearer $ACCESS_TOKEN" \
                  -H "Content-Type: text/plain" \
                  --data-binary @output.txt \
                  "https://graph.microsoft.com/v1.0/sites/$SITE_ID/drives/$DRIVE_ID/root:/Software/Firmware/output.txt:/content"
                '''
            }
        }
    }
...
\subsection{QA}
\begin{lstlisting}
Step 1:  Generate an SSH key on the Jenkins server
...
Do NOT add a passphrase, or Jenkins will need extra configuration.
...
Step 2 - Copy the public key to 10.0.45.101

Login to Jenkins server and run:

ssh-copy-id -i /var/lib/jenkins/.ssh/id_ed25519.pub username@10.0.45.101


If ssh-copy-id is not installed, do it manually:

cat /var/lib/jenkins/.ssh/id_ed25519.pub | ssh username@10.0.45.101 "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"


Replace username with the actual user on 10.0.45.101.

Step 3 - Test SSH manually on the Jenkins server

On Jenkins server terminal:

sudo -u jenkins ssh username@10.0.45.101


If you can log in without password, SSH is ready.

Step 4 - Add SSH key into Jenkins Credentials
...
Username: username on 10.0.45.101

Private Key: Enter directly

Paste the content of your private key:

cat /var/lib/jenkins/.ssh/id_ed25519

Passphrase is empty unless you added one.

Save.

Step 5 - Use SSH in Jenkins pipeline
        ...
        stage('SSH test') {
            steps {
                sshagent(credentials: ['my-ssh-key']) {
                    sh 'ssh username@10.0.45.101 "hostname"'
                    sh 'scp myfile.txt username@10.0.45.101:/home/username/'
                }
            }
        }
    }
...
\begin{lstlisting}
git rev-parse --resolve-git-dir /var/lib/jenkins/caches/git-100d3e5823c08e3aa936f360707c092e/.git # timeout=10 ERROR: Workspace has a .git repository, but it appears to be corrupt. hudson.plugins.git.GitException: Error performing git command: git rev-parse --resolve-git-dir /var/lib/jenkins/caches/git-100d3e5823c08e3aa936f360707c092e/.git

Delete the corrupted Jenkins Git cache directory.

Linux command:
sudo rm -rf /var/lib/jenkins/caches/git-100d3e5823c08e3aa936f360707c092e


Or remove the whole cache folder (Jenkins recreates it automatically):

sudo rm -rf /var/lib/jenkins/caches/*
...
Or from Jenkins web UI:

Manage Jenkins -> Restart Safely

Then rescan your multibranch pipeline.
\end{lstlisting}


\subsection{Docker}
docker can work with VS code very well.  inside a docker, you have source code, you want to edit, compile and debug it . 

install Dev container and contailer tool extension to you vs code.
\begin{itemize}
\item in you host machine, build a dir /project, then build a file
\begin{lstlisting}
.devcontainer/devcontainer.json  //build this file in your host project dirctly
{
  "name": "Use image 22",
  "workspaceFolder": "/home/dev/prj" //container project name
  "image": "img:22.22",  
  "settings": {
  ...
  "remoteUser": "root"           // or whatever user exists inside your image
}
\end{lstlisting}
F1 --> Dev Container: reopen in Container.  when you click left bottom to disconnet, container is still there. when you click left bottom to close remote connection, container is still there. container will be there even you reboot your system. so you can just use one container. 

\item you can build task.json inside vs Code, if compile is easy, just open terminal inside vs, run make clean and make

\item For embedded, we usually use remote gdb, (because we have source code avail in the crsso-compile-host machine, not in the real device.) In real device, run gdbserver :1234 app, then inside container project, build launch.json,  The detail has been introduced in gdbserver section. the same procedure, then press F5, select a configuration. The whole program will stop in the first line of main, then you can press next to continue your debug procedure.   
\end{itemize}
...
\begin{lstlisting}
sudo docker tag u22owrt22:22.22 nexus.***.com/repository/app/iot/u22owrt22:22.22
yan@yan-VMware-Virtual-Platform:~$ sudo docker push nexus.***.com/repository/app/iot/u22owrt22:22.22
...
\subsection{gcc}
\subsubsection{gcc basic}

	\par pkg-config is a helper tool for compilers. It tells your compiler: where headers are (-I), where libraries are (-L) and what to link (-l).
\begin{lstlisting}
gcc main.c -I/usr/include/libpng -L/usr/lib -lpng
gcc main.c $(pkg-config --cflags --libs libpng) #use pkg-config 
#from /usr/lib/pkgconfig/libpng.pc
\end{lstlisting}

	\par conan installs library and generates .pc file, then pkg-config reads it. 

   \par You can use md5sum to judge if two exes are the same. but if you use gcc, when you compile a.c with -g, it will include directory name, so the two binary executable on the different two computers maybe are different( due to the different directly, maybe different time stamp). 
...
\begin{lstlisting}
The first place the dynamic linker should
...
NOTE As is typical of many of the Linux configuration files that you've seen, ld.so.conf
may in turn include a number of files from a directory such as /etc/ld.so.conf.d.
Each line in ld.so.conf (or in the files it includes) is the name of a directory
that you want to include in the cache. The list of directories is usually
...
When the dynamic runtime linker encounters this variable, it often
...
\subsubsection{md5sum binary}
you can also use strip -s to strip the old binary, then use strip -s to strip the new compiled binary, then use md5sum to compare them. If the result is same, then they are the same. new compiled binary includes a lot of d a, I don't know why?

The weird ones: \$a, \$d, etc.

These are assembler-generated local symbols inserted by gas (GNU assembler):

\$a → start of a code (“absolute”) range.

\$d → start of a data range.

\$t → start of text (sometimes).

They are used internally for things like relocation, debugging, and disassembly alignment.
They don’t represent real functions or variables you wrote.

So in your case:

The \$d / \$a symbols are just markers inside .text to help the assembler / debugger understand code boundaries.

\begin{lstlisting}
0003a088 l     O .rodata	00000014              __func__.12
000334c4 l     F .text	00000294              tcpClientState
000334f4 l       .text	00000000              $d
0003350c l       .text	00000000              $a
000335e8 l       .text	00000000              $d
000335f8 l       .text	00000000              $a

objdump -t --no-show-raw-insn your_binary | grep -v '\$[ad]'
\end{lstlisting}
\par use below switch disable debug information and random value in binary. If binary is linked by many obj file, you can use md5sum for each obj file. 

\begin{lstlisting}
gcc  -g0 -frandom-seed=12345 -Wl,--build-id=none
\end{lstlisting}

\par, if you just change source code by delete some disable marco, such as \#if 0, new binary and old binary should be same. but make sure that you file doesn't include \_\_LINE\_\_, and assert (if you use two different directories.) comment these place, then you will get the same md5sum obj file or binary file.
\begin{lstlisting}
#define logDebug(...) clog_debug(__FILE__, __LINE__, CONTROLLER_LOGGER, __VA_ARGS__)
#define logDebug(...) clog_debug(__FILE__, 1234, CONTROLLER_LOGGER, __VA_ARGS__)
\end{lstlisting}

\par, use md5sum for each obj file, if they are different. 1) maybe \_\_LINE\_\_, 2) maybe include assert. You can use gcc -E to output source.i file to compare in source code level.   


\section{debug}
\par A few important principles:
AI is good hint, but don't fully trust it. it give you some hint, then use some keyword google, if the keyword has not a lot, that is AI hallucination.  

\par In opensource project, go to github, then search issue.  it's good place to find solution. especally for open source porject, such as openwrt


\subsection{memory}
below code will cause "illegal instuction" on openwrt, raspberry and ubuntu. The reason is free after write.  

\begin{lstlisting}
{header}  {data}
xxxxxxxxxxoooooooooooooooooooooooooooooo
          |p is here
free after write, it will overwrite next chunk {header}, the, when second free call, it will cause illegal instruction. 

The below code: p and q  has the same addree, because when you free q, then q will resue this address.             
...
int main(void) {
    ...
    // Step 3: corrupt the freed block by overflowing it
    // This overwrites malloc's internal bookkeeping
    strcpy(p, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
    printf("Heap metadata corrupted!\n");

    // Step 4: allocate again (may reuse same block)
    char *q = malloc(16);
    if (!q) {
        perror("malloc");
        return 1;
    ...
    strcpy(q, "world");
    printf("Allocated q: %s\n", q);

    // Step 5: free the corrupted block
    printf("Now freeing q... expect crash (Illegal instruction)\n");
    free(q);   // should SIGILL or abort due to corrupted metadata
...
Different between segmenation fault and illgal instruction.

Segmentation Fault (SIGSEGV)

Happens when your program accesses memory it’s not allowed to (invalid read/write/execute).
The CPU raises a page fault, and Linux delivers SIGSEGV.

Example 1 – invalid pointer dereference
\begin{lstlisting}
#include <stdio.h>

int main() {
    int *p = NULL;
    *p = 42;   // writing through NULL: invalid memory access
    return 0;
}
\end{lstlisting}


 This will crash with Segmentation fault (SIGSEGV) because address 0x0 isn’t mapped.
Example 2 – buffer overflow past mapped memory
\begin{lstlisting}
#include <stdlib.h>
int main() {
    char *buf = malloc(4);
    buf[1000] = 'X';  // way past the allocation
    return 0;
}
\end{lstlisting}

When you write far enough, you’ll hit an unmapped page → SIGSEGV.

Illegal Instruction (SIGILL)

Happens when the CPU tries to execute an instruction that doesn’t exist or isn’t allowed.
The CPU doesn’t understand the opcode, so Linux delivers SIGILL.

Example 1 – executing garbage as code
\begin{lstlisting}
int main() {
    void (*f)() = (void(*)())"\xFF\xFF\xFF\xFF";  // invalid machine code
    f();  // jump into garbage
    return 0;
}
...
The CPU sees 0xFFFF... (not a valid instruction) → Illegal instruction (SIGILL).

Example 2 – calling ud2 (explicit invalid opcode)

On x86:
\begin{lstlisting}
int main() {
    __asm__("ud2");   // guaranteed invalid opcode
    return 0;
}
\end{lstlisting}

Always aborts with Illegal instruction (SIGILL).

\subsection{sannitize}
what sanititize can do and can't do:
\begin{itemize}
\item Heap buffer overflow (read/write past malloc’d memory)

\item Stack buffer overflow (local arrays overrun)

\item Global buffer overflow (overflows in global/static arrays)

\item Use-after-free (accessing memory after free())

\item Use-after-return (stack memory accessed after function returns)

\item Double free / invalid free (freeing same pointer twice)

\end{itemize}

sanitize can't do
\begin{itemize}
\item For example, wrong pointer arithmetic that points to a valid allocated memory region — ASan won’t detect it if you don’t actually overrun redzones.

\item Resource leaks other than heap memory Open file descriptors, sockets, GPU memory — ASan doesn’t detect these.
\end{itemize}
...
halt_on_error=0  keep running after error.
...
use after free
heap overflow. There are three parts: 1) stack of where to alloc 2) stack of where to overflow 3) heap-buffer-overflow
\begin{lstlisting}
int main(int argc, const char *argv[]) {
...
\begin{lstlisting}
=================================================================
==697563==ERROR: AddressSanitizer: attempting double-free on 0xffff7ef007b0 in thread T0:
    #0 0xffff82e9ebbc in __interceptor_free ../../../../src/libsanitizer/asan/asan_malloc_linux.cpp:123
    #1 0xaaaabdea0980 in main /home/pi/a.c:8
    #2 0xffff82cace14 in __libc_start_main ../csu/libc-start.c:308
    #3 0xaaaabdea0820  (/home/pi/example_df+0x820)

0xffff7ef007b0 is located 0 bytes inside of 4-byte region [0xffff7ef007b0,0xffff7ef007b4)
freed by thread T0 here:
    #0 0xffff82e9ebbc in __interceptor_free ../../../../src/libsanitizer/asan/asan_malloc_linux.cpp:123
    #1 0xaaaabdea0978 in main /home/pi/a.c:7
    #2 0xffff82cace14 in __libc_start_main ../csu/libc-start.c:308
    #3 0xaaaabdea0820  (/home/pi/example_df+0x820)

previously allocated by thread T0 here:
    #0 0xffff82e9ee7c in __interceptor_malloc ../../../../src/libsanitizer/asan/asan_malloc_linux.cpp:145
    #1 0xaaaabdea0910 in main /home/pi/a.c:5
    #2 0xffff82cace14 in __libc_start_main ../csu/libc-start.c:308
    #3 0xaaaabdea0820  (/home/pi/example_df+0x820)

\end{lstlisting}

stack overflow  why run directly report Bus error?  Accessing invalid physical memory. E.g., pointer to memory that doesn’t exist or has been unmapped.

Difference from segfault:

Segmentation fault (SIGSEGV):  invalid memory protection (accessing memory you aren’t allowed to).

Bus error (SIGBUS): invalid alignment or hardware memory access (can’t physically perform the access).
\begin{lstlisting}
        void overflow(){
                char buf[8];
                strcpy(buf, "This aaaaaaaaaaaaaaaaaaa");
        }
...
==705837==ERROR: AddressSanitizer: stack-buffer-overflow on address 0xffffdfb3dea8 at pc 0xffffa3839ecc bp 0xffffdfb3d5e0 sp 0xffffdfb3d638
WRITE of size 25 at 0xffffdfb3dea8 thread T0
    #0 0xffffa3839ec8 in __interceptor_memcpy ../../../../src/libsanitizer/sanitizer_common/sanitizer_common_interceptors.inc:806
    #1 0xaaaad35d0ac4 in overflow /home/pi/a.c:7
    #2 0xaaaad35d0b28 in main /home/pi/a.c:11

\end{lstlisting}


It detect stack use after return . you can see an example in the ref "Memory error checking in C and C++: Comparing Sanitizers and Valgrind",  in my unbutu, you don't need run
\begin{lstlisting}
ASAN_OPTIONS=detect_stack_use_after_return=1 ./uar
\end{lstlisting}
but just run ./uar, it will report below 
\begin{lstlisting}
=115965==ERROR: AddressSanitizer: stack-use-after-return on address 0x77b560100020 at pc 0x5590170de345 bp 0x7ffd7af2dc30 sp 0x7ffd7af2dc20
READ of size 4 at 0x77b560100020 thread T0
    #0 0x5590170de344 in g /home/yan/test/d.c:7
    #1 0x5590170de366 in main /home/yan/test/d.c:10
\end{lstlisting}

 
ASan is much faster (2x slowdown vs 20–50x for Valgrind). Valgrind works on uninstrumented binaries, ASan needs recompile. ASan gives more precise error location.

\subsection{valgrind}

\subsubsection{basic}
different with gdb and gdbserver, valgrind can only run locally. 

it can deal with double free, write after free, and memory leak and 

It detects out-of-bounds reads/writes on malloc’d blocks.
\begin{lstlisting}
...
==17468==    at 0x103C4: main (a.c:7)
==17468==  Address 0x48d303c is 0 bytes after a block of size 20 alloc'd
==17468==    at 0x489AC88: malloc (vg_replace_malloc.c:381)
==17468==    by 0x103AF: main (a.c:5)
\end{lstlisting}


It detects writes to already freed memory.
\begin{lstlisting}
...
*ptr = 42;
 ...
 Invalid write of size 4
==14474==    at 0x10430: main (a.c:7)
==14474==  Address 0x48d3028 is 0 bytes inside a block of size 4,000 free'd
==14474==    at 0x489DCF8: free (vg_replace_malloc.c:872)
==14474==    by 0x10427: main (a.c:6)
==14474==  Block was alloc'd at
==14474==    at 0x489AC88: malloc (vg_replace_malloc.c:381)
==14474==    by 0x10417: main (a.c:5)
==14474==
==14474== Invalid read of size 4
==14474==    at 0x10438: main (a.c:8)
==14474==  Address 0x48d3028 is 0 bytes inside a block of size 4,000 free'd
==14474==    at 0x489DCF8: free (vg_replace_malloc.c:872)
==14474==    by 0x10427: main (a.c:6)
==14474==  Block was alloc'd at
...
==13836==    at 0x489DCF8: free (vg_replace_malloc.c:872)
==13836==    by 0x103F7: main (a.c:7)
==13836==  Address 0x48d3028 is 0 bytes inside a block of size 4,000 free'd
==13836==    at 0x489DCF8: free (vg_replace_malloc.c:872)
==13836==    by 0x103EF: main (a.c:6)
==13836==  Block was alloc'd at
==13836==    at 0x489AC88: malloc (vg_replace_malloc.c:381)
==13836==    by 0x103DF: main (a.c:5)
\end{lstlisting}


It detects Wild stack pointers
\begin{lstlisting}
int *p; 
*p = 42;   // uninitialized local pointer

= Use of uninitialised value of size 4
==12994==    at 0x1037C: main (a.c:7)
\end{lstlisting}
...
below can't guarantee to detect by valgrind 



Pointer arithmetic within same heap block
\begin{lstlisting}
int *p = malloc(100);
int *q = p + 20;  // valid
q[-1] = 42;       // technically underflow, but may hit within allocator's padding
\end{lstlisting}

Sometimes this doesn’t trigger, because you’re still writing inside allocator red-zone/padding that isn’t marked as “no-access”.

Global/static array out-of-bounds
\begin{lstlisting}
static int buf[10];
buf[20] = 42;   // buffer overflow

\end{lstlisting}


Valgrind usually cannot detect this, because it only tracks heap allocations, not static or stack buffers.

Stack out-of-bounds
\begin{lstlisting}
void f() {
    int buf[10];
    buf[20] = 42;  // stack smash
}

\end{lstlisting}
Memcheck cannot detect it (it doesn’t place redzones around stack locals).
Tools like AddressSanitizer (ASan) are better here.

Memory-mapped hardware/I/O regions
f your pointer points to MMIO space (e.g., /dev/mem), Valgrind assumes it’s valid — it won’t flag invalid accesses.
...
\textbf{Rule of thumb}
\begin{itemize}
\item Heap bugs:  Valgrind works great.

\item Stack/global buffer bugs: Use ASan or compiler instrumentation (-fsanitize=address).

\item Performance-critical hardware/MMIO code: Valgrind is not suitable.
\end{itemize}
...
In openwrt, it need unstripped libc.so. I have created libc.so with debug symbol and name it with libc.so.unstrip.  go to openwrt /lib, then cp libc.so to libc.so.old, then cp libc.so.unstrip to libc.so (don't change libgcc\_s.so.1, it will crash the system.) then run below command.  
\begin{lstlisting}
valgrind --leak-check=full --soname-synonyms=somalloc=NONE /root/example_free_free
\end{lstlisting}

how to generate libc.so with symbol? 
\begin{lstlisting}
make menuconfig
Global build settings -- *compile packages with debugging info

make package/libs/toolchain/compile V=s

then it will generatge build_dir/toolchain-arm_cortex-a5+vfpv4_gcc-11.2.0_musl_eabi/musl/lib/libc.so //this is libc with debug symbol
size is 

//this procedure maybe not accurate, but make menuconfig first, then run make package/libs/toolchain/compile is right. just check if libc.so has been found in build_dir directory. 

4298928 Aug 27 22:23 libc.so*
md5sum libc.so 
a7b841dc5b8fb64bb5f25ae52ff84527  libc.so //md5sum maybe not correct due to different directory name on different computer name. 
\end{lstlisting}


why need --soname-synonyms=somalloc=NONE? 
...
musl’s malloc is simpler and doesn’t always abort cleanly with “double free” or “invalid size” like glibc.

When heap metadata is corrupted, the allocator may follow bogus pointers → CPU decodes random memory as instructions → SIGILL.
...
3) get the core dump file, then 

4) gdb exe core.dump

5) set sysroot /usr/local/rb-debug-x86_64/sysroots/aarch64-oe-linux
info sharedlibrary
...
both .test should have the same architecture. remote gdb has two advantages 1) local has all source code and git repo support,  2) remote has all necessary hardware configuration. You only need to compile in local and then copy binary to server(test dvice) remote
\begin{lstlisting}
sudo gdbserver :1234 ./test

sudo gdb ./test, 
then target remote <server>:1234
break main
c
n
\end{lstlisting}
...
You can use vscode (ssh) as frontend of gdb, then use gdb to connect gdbserver to a ranger. That is very useful for embedded server. 

\begin{itemize}
\item local machine: (vscode, use ssh-remote remote-ssh:connet to hose)

\item compiler device:(install gdb and all the source code, then run make file to produce binary)

\item test device: (with all necessary hardware, install gdbserver, copy binary from compiler device, then, run sudo gdbserver :1234 ./binary, don't forget sudo command if you binary need to write to /usr/bin ) 
 
\end{itemize}


Add below to the launch.json
...
{
            "name": "Remote Debug",
            "type": "cppdbg",  // install c/c++ extension in ssh:remote ...
            "request": "launch",
            "program": "/home/dev/binary",
            "args": [],  // "args": ["arg1"],  
            "stopAtEntry": true,
            
            "cwd": "/home/dev",
            "environment": [],
            // @see https://stackoverflow.com/questions/40033311/how-to-debug-programs-with-sudo-in-vscode
            "externalConsole": false,
            "MIMode": "gdb",
            "miDebuggerPath": "/usr/bin/gdb", 
            "miDebuggerServerAddress": "10.0.40.66:1234", 
            "setupCommands": [
                {
...
\par 
\begin{lstlisting}
To avoid password, c:\Users\yzhao\.ssh
type id\_ras.pub, then copy and paste it to target board .ssh\authorized\_keys
\end{lstlisting}


\par Three common used pattern when use ssh:
\begin{itemize}
...
When you paste python code, you need to keep indent, goto Options->Configure TeXstudio->Editor->Indention mode, change it to keep indention. 
...
zip all source code with CMakeLists.txt, then in compiler explorer, you can add Tree(IDE mode), check CMake, then it will run multi C++ files. It's very useful. 

\par panel can be float inside browser, just like in other IDE. 

\par 
\subsection{VSCode}
For embedded development, you can use VS code to remote access your board, then you can edit code and compile code directly in VS code. You can also use copilot to help you write code. 
\begin{itemize}
	\item In windows, install the newest version of VS code, then install Remote-SSH extension, then you can use it to remote access your board. 
	
	\item If remote ssh doesn't work, map remote directory to local by sshfs, then open local directory in VS code. You can also use rsync to sync code between local and remote.
	
	\item install winfsp and sshfs-win, then in windows explorer, input \\sshfs.r\yan@10.0.40.96\data\yan. that all. it will map remote directory to a local dirver, Z: Y: into the windows explorer. Then you can open this directory in VS code. You can also use rsync to sync code between local and remote. if deal map Z: still exist, run below command to stop it. 
	\begin{lstlisting}
		start powershell with admin right, then run below command to stop map Z:
		 taskkill /f /im explorer.exe
		 start explorer.ext 
	\end{lstlisting}
	
	\item A better way is to use wsl 
	\begin{lstlisting}
	sshfs   -o allow_other   yan@10.0.40.52:/home/yan/test   /home/yzhao/v18
	then in explorer, input 
	\begin{lstlisting}
		\\wsl$\Ubuntu-20.04\home\yzhao\v18.
	\end{lstlisting}
	
	\end{lstlisting}

	\item Then open vs code in windows, manually input \\wsl$\Ubuntu-20.04\home\yzhao\v18. because The vs code can't find this wsl directory automatically. 
...
#search how to get all includes path.
{
    "configurations": [
        {
            "name": "Cross",
            "includePath": [
                "${workspaceFolder}/**",
                "/usr/local/rb-debug-x86_64/sysroots/aarch64-oe-linux/usr/include/c++/9.3.0",
                "/usr/local/rb-debug-x86_64/sysroots/aarch64-oe-linux/usr/include/c++/9.3.0/aarch64-oe-linux",
                "/usr/local/rb-debug-x86_64/sysroots/aarch64-oe-linux/usr/include/c++/9.3.0/backward",
                "/usr/local/rb-debug-x86_64/sysroots/x86_64-qtisdk-linux/usr/lib/aarch64-oe-linux/gcc/aarch64-oe-linux/9.3.0/include",
                "/usr/local/rb-debug-x86_64/sysroots/aarch64-oe-linux/usr/lib/aarch64-oe-linux/9.3.0/include",
                "/usr/local/rb-debug-x86_64/sysroots/x86_64-qtisdk-linux/usr/lib/aarch64-oe-linux/gcc/aarch64-oe-linux/9.3.0/include-fixed",
                "/usr/local/rb-debug-x86_64/sysroots/aarch64-oe-linux/usr/include/"
            ...
            "compilerPath": "/usr/local/rb-debug-x86_64/sysroots/x86_64-qtisdk-linux/usr/bin/aarch64-oe-linux/aarch64-oe-linux-g++",
        }
    ],
    "version": 4
}
...
VS can install copilot extension. There is yzhao@mycompany and job pw. there are three number in the end in pw.
...
1)editor enable suggestion is main switch, don't turn off it. 
2) disable ghost text in copilot, 
3) ctrl+K,s Then give "trigger inline suggestion" keyboard shortcut.  alt+\. That is the most easy way to use it.


\begin{enumerate}
	\item tab :accept
	
	\item ctrl + -> : accept part, don't need to type tab, otherwise, it will accept whole suggestion, but you just want to accept part of it. After control+riht arrow, just keep typing.
