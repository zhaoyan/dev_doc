...
\end{enumerate}
		...
		\item \textbf{Rule7: c command is more useful than d command, 1)it will go intp insertmode directly, 2)and all the action after that can be repeated by . command}
		...
		\item By now, I configure cursor in different shape in insert and normal mode. By adding two command in my vimrc file.
\begin{verbatim}
...
\end{verbatim} 
		...
		\item dot is interesting thing in vim:
				\begin{enumerate}
						\item In normal mode,dot represent repeate last command.
						\item In reg. dot preprest last insert content. If you use dot, it will remember a command before you go into the insert mode. If you use ".p, It just paste your insert content at current position. 
...
\item For text object,  \textbf{delete around, change inside}. Detail can be found in pratical vim tip 52. 

\item \textbf{VimTex} plugin will add new text object, such as vimtex add "e" and "c" two text objects. 

\item \textbf{Surrord} plugin add another control scope, such as "s" beside "a" and "i". 
\end{itemize}
...
\item marks lists basic:
		...
		\item Double single quote command will help you to jump back origin position, but still in normal mode, gi command will return back to the position where you exit insert mode, and return back to insert mode. It's very uesfule command. 
\end{enumerate}
...
\item search command in VIM:
		\begin{enumerate}
				...
				\item Press F4 to toggle highlight research result. 
		...
		\end{enumerate}
				...
				\item Another subtopics is search scopes.
						\begin{enumerate}
								...
								\item :vimgrep and :lvimgrep difference lies in write to location list or quickfix list.
								...
								\item For :lv, you can use **/*.cpp to search all .cpp files recursively. You also can input complex regex manually, So :lv is more flexible than $\backslash$vv. $\backslash$vv is more convenient than :lv. 
						\end{enumerate}
...
\item tricks about edit command: 
		\begin{enumerate} 
				\item  Edit commands "d,c,y", \textbf{d keep in the normal mode. c go to insert mode. y keep contents.} All edit command "d,c,y" can follow motion, it give edit command a "operator scope". For example, "d2w" will delete next two words. "d3j" will delete next 3 lines. "ct;" will delete every thing until ";", it's very useful for C++ language. 
				\item \textbf{d,c can follow EasyMotion command, but y can follow EasyMotion command.}

				\item C delete to the end of line,and go into insert mode. D is the same, but stay in the normal mode. S delete the whole line, and then go into the insert mode. X delete previous character. \textbf{remember(C,D) and (S,dd).}
...
\begin{itemize}
		\begin{enumerate}
				...
				\item gP is useful when you want to paste multi-line text several times. See "Practical VIM tip 62"
...
\begin{enumerate}
		...
		\item For ctags and cscope and doxgen, you can download my project template cmake-test from github. There are src-tool.sh in src directory, you can use them automatically.  
