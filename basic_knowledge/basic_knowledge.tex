...
\begin{itemize}
		\item There are two basic shelles, one is bash and the other is tcsh. They can be change by calling \linuxcommand{bash} or \linuxcommand{tcsh}.  You can use echo \$0 to see which shell you are using right now. By now, a new shell is zsh, It support more features. 
...
\end{lstlisting}
		...
		\includegraphics[scale=0.4]{pics/check.png}
...
\end{enumerate}
	...
	\item both reset and commit has key point. \textbf{See if they follow path?} , if follow path, the main purpurse is overwrite. 
...
\begin{itemize}
	\item When you add, You should know below procedure. 
	\begin{enumerate}
...
\end{verbatim}
	...
	\item For example, for latex,  Only *.tex *.bib( reference)  and /pic directory are useful, You need to add them manually.
	...
	\item Both(commit --amend) and (reset... commit again) will produce new commit. In fact, any commit sha include current operation and second operation will be different. 
...
\item save, then load,  if import then export. They are different, you can google them.
save from image, so it includes layer information, but export from container. 

\item if you want to push, you need to tag it first,  such as ubuntu:18.04,  18.04 is a kind of tag, don't forget these number.  Once you use this tag name to push, use the same tag name to pull for docker server, such as docker hub. 
