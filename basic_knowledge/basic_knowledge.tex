  ...
  \item for File and Dir, "rwx" has different meaning. x for Dir means you can come into
      ...
      \item \linuxcommand{chown chgrp} are very useful when you copy files to other
       ...
       \item \linuxcommand{chmod} command follow[ugoa][+-=][rwx], for example,
           ...
           u(owner), g(group), o(other) and a(all).
\end{itemize}
...
\begin{itemize}
     ...
     \begin{enumerate}
     	  ...
     	  \item \linuxcommand{find . -size +50M -size +20M}  c:bytes, k:kbyte, M:Mbyte G: b:512-byte blocks. 
...
\item single quotes preserves the literal value of each character.  In double quotespreserves the literal value of all characters within the quotes, with the exception of ‘\$’, ‘`’, ‘$\backslash$’, and, when history expansion is enabled, ‘!’. 
\begin{verbatim}
echo '$PATH `pwd` '
echo "$PATH `pwd` \$PATH"
\end{verbatim}
...
\item echo \$0 will show "bash", it's not login shell. if it shows "-bash", it is login shell.  
	...
	\item In mint, maybe in your home directory, there is no .bashrc file, so you need to create one and add \\
	export PATH=\$PATH:/home/yan/openuh-install/bin \\ then exit the current terminal and restart a new one. Then use echo \$PATH to see if the directory has been added.
...
\item \linuxcommand{vim -v} will open read-only files, ZZ will write and quite vim. 
...
\subsubsection{basic command}
	...
	\begin{center}
		...
		move & \\
		\hline
		1G and G  &  begin and end of document\\
		\hline
		 ctrl+b and  ctrl+f & previous and next page\\
        \hline 		
		0 and \$ & begin and end  of line \\
	   	\hline 		
		w and b & next and previous  of word \\
		\hline 		
		\% & goto mate \\
	   	\hline 		
		3w  4j and 15gg& it supports number+command mode \\
...
\begin{center}
		...
		delete & \\
		\hline 
		x  and r & delete and replace one character\\
		\hline 
		dw  and cw & delete whole word\\
		\hline 
		dd   & delete current line\\
		\hline 
		3dd and 2dw   & support number+command \\
		\hline 
		u and . (ctrl+r)  & under and redo last command \\
