...
\subsection{Motion command}
	...
	\item <C-o> will return back last jump positio, <C-^> will switch back to previous file. They are useful command you can use them. 
...
\item marks lists basic:
		...
		\item Captive letter will mark multi file position.
		\item There are single quote command, backtick command. One will jump to exact mark, Other will jump to mark line. They need follow a mark name.

		\item gi will return last position you exit insert mode. 
		\item `. will return your last edited position. 
		\end{enumerate}
\item \linuxcommand{:lv word \%} will search all word in current file. It will show all the result in position list. 
		\begin{enumerate}
				\item position list is assocaited with current window, and quickfixe list is global.
...
\begin{enumerate}
		\item \linuxcommand{find . -name "*.c" | xargs git add} to add all source code to git, Then you can build a branch test. 
		...
		\item Any time you eidt a source code, run \linuxcommand{:!cscope -Rbq}, then run :cs add cs<tab>. add cscope.out file. Then you can use cscope command to jump.

		\item set path , then you can jump to header file by gf command, then use <C-^> to return. A more usefule command is <C-w>f command, it will open header file in a new windows.  
		...
		\item use cscope, detail can be found in cscope section.  

		\item First, use F8 to open tagbar windows, it shows all global varaible and function in this files.
		...
		\item gD and $[$i have the same meaning, gD will jump to it, and $[$i just show this line below. They don't have semantic measning, just the first line in the current file. <C-]> is just like :tag, only support global scope. \textbf{If you want to jump to the definiation of local varialbe, 1) use # or * 2)use :lv <C-r><C-w>\% to open a position list window, it will list all word in the current file. 3) use vimgrep plugin}
		...
		\item For local varialbe, you can use * and \# comand to search it. After that, you can press F4 to toggle hisearch. $[$i will jump to the first position of current cursor word in this file. For some common varaible, such as i,j, It's not right. so \# method is better.
...
\begin{tabular}{p{0.3\textwidth}|p{0.3\textwidth}|p{0.3\textwidth}}
		...
		where & :Cwhere & W \\
		\hline 
		frame n & :Cframe n & void \\
		\hline 
		up down & :Cup :Cdown & <C-u> <C-d> \\
		\hline
		\end{tabular}
\item varaible:
\begin{tabular}{p{0.3\textwidth}|p{0.3\textwidth}|p{0.3\textwidth}}
		...
		print &:Cprint  & F9 \\
		\hline 
		set var  & :Cset var & void \\
		\hline 
		info local &  & L \\
		\hline
		dbgvar expr & :Cdbgvar & void \\
		\hline 
		\end{tabular}
