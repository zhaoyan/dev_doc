...
set number	
	\end{verbatim}
\item Build a IDE, You need:
		\begin{enumerate}
				\item File explore: NerdTree and CtrlP
				\item Tag jump: tagbar, cscope and easyGrep, gD command and Ctrl+](need tags file)
				\item Make: quickfix
				\item Syntax: syntax
				\item Autocomplete: SuperTab, Vim-snippet, Ycm, Ctrl+p
				\item Debug pyclewn
				\item Comment: Nerdcomment
				\item Fold: For C/C++, just :set foldmethod=syntax, For python, use simpylFold plugin 
		\end{enumerate}
...
\item Visual Mark is also a good plug in, usage is very simple, mm visual mark, F2 and shift F2 navigate, Ctrl+F2 toggle it
...
\item reformat will perform action according to the previous lines indents. So before reformat, you can use == reformat the previous line of code to right indent.
	 ...
	 \item set number; syntax on and set ai;  whenever you hit Enter to start typing on the next line, vim automatically will indent to the same amount as the previous line.
		...
		\item For autocomplete, You can use SuperTag,Vim-Snippet, Ctrl+P and YCM four plugin. The previous three use Tab, and fourth use . and -> 

		\item For Syntax error use :Errors, For make error, use quickfix mode.
\end{itemize}

\subsection{Visual-Mark}
\begin{itemize}
...
\begin{itemize}
		...
		\item You can .tex directory, you can build a .chktexrc file and add below to it.
\begin{verbatim}
CmdLine
{
--nowarn 1
...
\end{verbatim} 
nowarn use to suppress some unnessary warning message.

\item You can add \verb!let g:syntastic_tex_checkers=['chktex']! to tell Syntastic use chktex to check latex.
\item When you use lacheck, you can't suppress warning. But you can add tex.vim file to after/ftplugin to deal with some tex type file. In this file, you can add let g:syntastic\_quiet\_messages to it, detail can be seen this file.
\item For latex document, there are a lot of unimportant error, You can use to 
