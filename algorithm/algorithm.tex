...
\chapter{Data structure} 
	...
	\par Data structure is known as ADT (Abstract Date Type),  which is independent of implementation. For example, stack can be implemented by array or linked list inside. Such as \texttt{std::stack} in STL library, It's implemented by \texttt{std::deque} default.   
...
auto bi = v.begin();
auto ei = v.end();
auto mi = next(bi, distance(bi, ei)/2 );

for(auto i = bi, j = prev(ei); i<mi; ++i, --j){
	iter_swap(i,j);
}
...
while(d-- >0 ){ //d-- >0 is good trick
  int temp = Array[0];
  for (int i = 0; i < n-1; i++){ //left open, right close, when i<n-1, it means that we need to access array[i+1]
    Array[i] = Array[i + 1];
  }
  Array[n-1] = temp;
}
...
//this is a good pattern, need to remember, l.splace(l.end(), ....)
\end{lstlisting}
...
\begin{itemize}
	\item Two sum. 
\begin{enumerate}
\item Answer 1: First Sort O(n*log(n)), then use two pointers. \textbf{for the two pointers, from two ends to the middle, use while(l<r) is a good strategy.} Two typical applications are reverse and the two sum. 
...
\end{lstlisting}
	...
	\par There are a few points you need to pay attention to when you implements dfs by iteration:
...
\begin{lstlisting}
 void BFS(vector<vector<char>>& grid, int i, int j){
	...
	q.push({i,j});
	grid[i][j] = '0'; //the first element also need to marked as visited. 
	while(!q.empty()){
		auto [i,j] = q.front();
		q.pop();
		cout<<i<<" "<<j<<endl;
...
\begin{lstlisting}
vector<int> v = {1, 1, 1, 1, 1, 2, 2, 3, 4, 1, 6, 6, 6, 7, 7, 7,7};
unordered_map<int, int> m;
for(auto e: v){
	m[e]++;
}

auto l = [](pair<int, int> &e1,  pair<int, int> &e2){
	return e1.second > e2.second;
};

priority_queue< pair<int, int> , vector<pair<int, int> >,   decltype(l) >  pq;
...
You need to perform k operations on nums. In each operation: Find the minimum value x in nums. If there are multiple occurrences of the minimum value, select the one that appears first.
Replace the selected minimum value x with x * multiplier.
Return an integer array denoting the final state of nums after performing all k operations.
\begin{lstlisting}
 std::vector v {2,1,3,5,6};
...
\begin{itemize}
	...
	\begin{enumerate}
		\item global varaible is just reference, you need ++ and -- to keep it. global can save stack size. 
		
		\item local variable don't need ++ and --, it save in the local stack, if we want to get level, you can use it, if just int, it also has the same size as reference. 
		
		\item \textbf{return value usually used in postorder traversal}. most of time return bool, int(size, lenth or), or tree node. 
	\end{enumerate}
...
\subsection{Basic}
	...
	\par \textbf{traverse all nodes is different with path from source node to target node.}
\begin{itemize}
	\item traverse use visited[] to avoid loop.
	
	\item if it's DAG, you don't need to use visited. for example, we don't use visited in tree. 
	
	\item For path, that should be DAG, in this way, we don't need use visited either. (if there is a loop, can't print a valid path)
	
	\item for path, if you want to use visited, it will select only one path pass a visited node, but skip others(maybe it's not what you want.)
	
	\item DFS is different with backtrack, one care about node(traverse), The bt care about different decision(select). Although they are looks like tree, but different, \textbf{in tree and graph, node are really exist, but in bt, node are not really exist, they generate by a selection, disappear by revoke}
\begin{lstlisting}
dfs(){
	path.push
	for(....){
...
\subsubsection{DFS}
	...
	\par DFS has three versions 1) recursive 2) iterate. 3) uniform version.
	
\begin{itemize}
	
	\item Recursive version: in this dfs, \textbf{you only need to check one place if unvisited. }. Also you should select dfs version first. 
\begin{lstlisting}[frame=single, language=c++]
DFS(current){
  visit current;
...
//Another better version is to put "is visited" into the dfs beginning.
//
DFS(current){
...
\begin{center}
	\includegraphics[width=0.7\linewidth]{pics/basic.drawio}
\end{center}


\par Why this basic conception is so important? Repeated permutation can be use in 0/1 backpack problem.  LCS problem.  
...
\begin{itemize}
	...
	\item 5 repetitive combination,   5 icecream, 3 spoon, (c(5+3-1, 3))
	
	\item distribute 10 candy to 3 kid, that is integer partition. result is (c(10+3-1, 2)). It's like above repetitive combination problem. thank there are 10 number, and two bar, put two bar in 12 position. The same question is: 10 same ball, put three bucket, that is same above quetions. 
	
\begin{lstlisting}
void distribute(int balls_left, int buckets_left, std::vector<int>& current) {
		if (buckets_left == 1) {
			// Only one bucket left: put all remaining balls here
			current.push_back(balls_left);
			
			// Print the result
			for (int val : current) std::cout << val << " ";
			std::cout << "\n";
		...
		for (int i = 0; i <= balls_left; ++i) {
			current.push_back(i);
			distribute(balls_left - i, buckets_left - 1, current);
			current.pop_back(); // backtrack
		}
	}
	...
	int main() {
		...
		distribute(total_balls, buckets, current);
	}
\end{lstlisting}		

	
	\item 10 different ball, put 3 different bucket, There are two different perspectives. One is from ball perspective, the other is from bucket perspective.  From ball perspective, the result is pow(3, 10).
	
	
	\item 10 differnt ball, split 3 equals. A specal case of above question, you can use ball perspective way, but it's low effecient. A better way is to use bucket perspectivce, we don't need to get different combination, for one bucket, get all subset of 10. the process can be repeat 3 time. So the result is 3*pow(2, n). much less than pow(3, 10)
\end{itemize}
...
\begin{itemize}
	\item O(n) is found max , min or sum. 
	
	\item O(n*n) is bubble sort
	
	\item O(n*n*n) is solutions for a multi-variable equation: 3x+4y+5z = 108, then get all x, y z. three kids split 10 candy. 
	
	\item O(n*logn) is quick sort. binary search. DC
	
	\item O($2^{n}$) print all subsequences.  subsequence and subset are different concepts, even though they sound similar. subsequence must keep the original order, but in subset, order doesn't matter.  subsequence mainly used in string or array, subset mainly used in set or list.
...
\subsubsection{When to Use Recursion (Despite the Overhead)}

\textbf{1. Clarity and Simplicity for Recursive Data Structures}

Some problems map naturally to recursion:

\begin{itemize}
	\item Linked lists
	\item Trees
	\item Graphs
	\item Recursive patterns (e.g., fractals, file systems)
\end{itemize}
...
This recursive tree traversal is clean, readable, and directly mirrors the structure of the data.

\textbf{2. Divide and Conquer Algorithms}

Recursive thinking fits naturally in algorithms like:

\begin{itemize}
	\item Merge Sort
	\item Quick Sort
	\item Binary Search
	\item Backtracking (e.g., solving Sudoku, N-Queens)
\end{itemize}

Even if they can be rewritten iteratively, the recursive form is often much easier to understand and implement.

\textbf{3. Mathematical Recursion / Definitions}

Some algorithms are defined recursively, and recursive implementations are more direct:

\begin{itemize}
	\item Factorial, Fibonacci
	\item Combinations / permutations
	\item Recursive dynamic programming (with memoization)
\end{itemize}


\subsection{When to Avoid Recursion}

\begin{itemize}
	\item Performance-critical code
	\item Large inputs (risk of stack overflow)
	\item Deep recursion without tail-call optimization
	\item Platforms with limited stack size (e.g., embedded systems)
\end{itemize}
...
\begin{table}[h!]
	...
	\begin{tabular}{|l|c|l|}
		\hline
		\textbf{Situation} & \textbf{Use Recursion?} & \textbf{Reason} \\
		\hline
		Tree traversal & Yes & Natural fit, clean syntax \\
		Graph DFS/BFS & Maybe & DFS = recursion; BFS = queue-based \\
		Deep iteration with no recursion & No & Stack-safe, faster \\
		Divide \& conquer (e.g., mergesort) & Yes & Matches conceptual model \\
		Large input size ($>10^4$ elements) & No (usually) & Stack depth issues \\
		\hline
	\end{tabular}
	\caption{When to use recursion: rule of thumb}
\end{table}

\subsection{Conclusion}

While recursion has performance drawbacks, it is still highly useful for expressing problems cleanly and matching natural structures. Use it when it makes code clearer, but be mindful of stack depth and convert to iteration if performance or memory becomes an issue.
...
\par Two pointers are very popular algorithms in many interview questions. 
...
string palindrome(string& s, int l, int r) {
	//from l go to left, from r go to right. 
	//<----l  r---->
...
\end{lstlisting}
	...
	\par \textbf{Basic rules:}
\begin{enumerate}
	\item If a subarray or substring has constraints, if not, use two for brute force, an example is leetcode 2913
	
	\item if there is constraints. double pointers, leetcode 3, 76, 567 and 3258.
	
	\item Do we need map to store substring information?  for 3, 76, and 567. we need it. For 3258, we don't need map to store character information in substring, that is very important clue, for 3, we need to judge if there is repeated character, so we need a map. A similar questions can be found.
\end{enumerate}
...
void MergeInPlace(vector<int>&ivNums, int lo, int mid, int hi){
	int i = lo, j = mid;
	while (i<mid && j<hi){ //stop when the first or latter half is empty
...
%	\par Identify the "state": This refers to the variables that change between the main problem and subproblems. 
%	
%	\par Determine the "choices": These are the actions that lead to changes in the state. 
%	
...
Basic strategy: (Four steps!)
\begin{itemize}
	\item If it's recursive? if no, if it's iterate? if yes, what atomic operation is?
	
	\item Define all necessary states in one atomic operation? what need? what change? 
	
	\item Init condition and end condition? init condition should be same in each iteration. 
	
	\item Iterate or recursive? what value can be saved to avoid iterator again? 
\end{itemize}

Pay more attention to below points when you write C++ code:

\begin{itemize}
	\item \textbf{The first step, consider the edge case!}
	
	\item Write slowly, try you best to write zero bug code. 
	
	\item Prefer to use std container, algorithm and views, especially views:: if the compiler supports it. 
		
	\item Use auto as many as possible. Must know if you need add reference \& or const when you use auto and structure bind. 
	
	\item prefer to use what you know and sure, not new feature which you are not quite sure. 
	
	\item If there are two options, ask first, then coding. 
\end{itemize}




\section{Basic data structure and algorithms}

\begin{itemize}
	\item Circular queue: 1) use \%, 2) when is full? front == k-1  3) when is empty? back== front 
	
	\item reverse(double pointer + iter\_swap) and rotate(based on reverse).
	
	\item reverse list. (three pointers) 
	
	\item inplace merege, just like reverse, need to know init and end condition, and in each step, what to do?
	
	\item multi merge based on heap.  
	
	\item remove and partition based on double pointer. remove (++ first, then copy) partition (swap first, then ++)
	
	\item sort, insert sort, selection sort, quick sort (based on partition) and merge sort. Quick sort based on partition, merge sort based on merge.
	
	\item shuffle (swap + random)
	
	\item matrix traversal.
\begin{enumerate}
	\item transpose and rotate,
	\item bottom to up
	\item Diagonal traverse
	\item matrix multiplication traverse
	\item spiral print
	\item DFS, and BFS.
\end{enumerate}
	
	\item tree traversal(pre-order, inorder and post-order)
	
	\item trie tree, just multi-branch tree, use pointer array, and is\_leaf. 
\end{itemize}	
	
\subsubsection{Math}
\begin{itemize}
	\item  Know the difference, 
\begin{lstlisting}
-1<<1  still -1
-1/2   equal 0
-3%2   equal -1	
\end{lstlisting}	
	...
	\item know gcd and lcm. How to get prime number. 
\end{itemize}
...
\begin{itemize}
	\item 4753, 47743. container methods summary.
	
	\item Some new containers, such as std::forward\_list, std::array and std::bitset and other a few other range-like views, such as std::span and std::string\_view.
	
	\item You can use erase for(vector, string) and contains(for associate) and starts\_with and ends\_with(for string) if compiler supports. 
	
	\item For std::array, the main interface are access, iterator and fill. std::span also has access, iterator and \texttt{first()}, \texttt{last()} and \texttt{subspan()}.
	
	\item how to construct? use std::vector/std::string.reserve. 
\begin{enumerate}
	\item iota
	\item fill, generate + mt19937
	\item () and \{\}
	\item from other container. 
\end{enumerate}
		
\end{itemize}

\subsubsection{construct and init}

\begin{itemize}

	\item No. Not all STL iterators support <. Only random access iterators do. when I iterate list, I can't use it<l.end(), only can use it !=l.end(),

	\item Know common container initialization:
\begin{lstlisting}
...
//3) build from different container.
set<int> s {1, 2, 3};
...
vector<pair<int, int> > v1{m.begin(), m.end()}; //vi[0] is pair<int, int>
vector v1(m.begin(), m.end()); // v1[0] is pair<const int, int> 

map<int, int> m {{1, 2}, {2, 4}};
vector v1{m.begin(), m.end()}; // That is not correct, because {} not ()


vector<pair<int, int> > v1{{1, 2}, {2, 4}};
map<int, int> m {v1.begin(), v1.end()};	
\end{lstlisting}

	\item Prefer to use range and reserve
\begin{lstlisting}
vector v1 {1, 2, 3};
vector v2;
v2.reserve(2*v1.size() );  // first reserve
v2.assign(v1.beign(), v1.end());  //range assign
v2.insert(v2.end(), v1.begin(), v1.end()); //range insert.	
\end{lstlisting}
...
\begin{itemize}
	\item for map, insert, emplace, emplace\_hint, try\_emplace difference. [] and insert\_or\_assign difference. Use more emplace if it is possible. 

	\item for different container, insert has different overload, for sequence, need a position for insert position, for associate, usually doesn't need insert position, but have a insert(hint, value)to keep consistent with sequence insert. Usually they all support range insert(iter1, iter2).

	\item erase+remove for vector, remove for list and erase for associatate. in C++20, vector has non-member erase and erase\_if. map has non-member erase\_if for easier use. std::erase and std::erease\_if for vector and string and deque

	\item fill and generate can be thought as another insert. 
\end{itemize}

\subsubsection{update}

major method to update key value or value value in associate container.  first erase(i++); then insert(i,e)

A better method is to use extract in c++23
\begin{lstlisting}
auto n = m.extrat()
n.key() = 3;
m.insert(std::move(n));	
...
\subsubsection{look up}
\begin{itemize}
\item find, count, contains, find\_if,  low/upper\_bound and binary\_search

\item all/none/any\_of return bool. 
\end{itemize}

\subsubsection{operations}

\begin{itemize}
	\item basic shuffle and sort operation. 

	\item Use list.splice in list to finish rotate. 

	\item range assign, construct and insert and erase

	\item min priority queue
\begin{lstlisting}
	priority_queue<int, vector<int>, greater<int> > min_pq;
\end{lstlisting}

	\item Some typical auto usages:
\begin{lstlisting}
for(auto &[f, s] :map1) //use & here, no need const, 

auto [it, ins] = map1.insert_or_assign(1, 3);

conat auto [f, s]  = ranges::remove() //no & here, use const for better security.

auto [fi, si] = multimap.equal_range(1); // struct binding, fi is first iterator
...
\begin{itemize}
	\item Own member functions:
	\begin{enumerate}
		\item string.find/rfind(ch/str, pos)
	...
	\item common functions: ::toupper, ::tolower, isupper, islower, isdigit, isalnum, isalpha, isspace, 
	
	\item to\_string and stol, stof, stoul, from\_chars and to\_chars is much faster.
	
	\item trim(use find\_if\_not and ::ispace) and split (getline + stringstream). 
	
	\item find + while pattern.	
\end{itemize}
...
\begin{itemize}
	...
	\item SIMD vs. Multithreading
	
	SIMD and multithreading improve performance in different ways:
	
	SIMD	Multithreading
	One core processes multiple data elements per instruction	Multiple CPU cores work simultaneously
	Good for arrays, matrices, image processing	Good for independent tasks or dividing large workloads
	Examples: AVX, AVX2, AVX-512, NEON	std::thread, std::jthread, OpenMP, TBB
	
	They are often used together. For example, a program may split an image across multiple threads, and each thread uses AVX or NEON instructions to process its assigned pixels efficiently.

	\item Use std::execution::par when:
	
	Processing millions of elements.
	each iteration is independent.
	the work per element is significant (sorting, numeric computation, image processing, etc.).
	You've verified that your standard library provides an effective parallel implementation.
	
	Avoid it when:
	
	The container is small.
	Operations modify shared state without synchronization.
	The algorithm depends on iteration order or has side effects that aren't thread-safe.
	
	Most algorithms with execution-policy overloads have one characteristic:
	
	They process every element (or large chunks of elements) independently, making them suitable for parallelization.
	
	Algorithms that do not support execution policies often rely on a sequential search or maintain a strict state between steps, such as:
	...
	These don't parallelize as naturally while preserving their semantics.
	
	Finally, remember that execution policies apply to the classic STL algorithms (e.g., std::sort), not the ranges algorithms (std::ranges::sort) in C++20/C++23.

	\item ACCMFS, RRRRSSSSFFPPCMT, HMSSS
	
	\item If compiler supports, use ranges::algorithm, pay attention 1) 4 math is not in ranges. 2) range return value are different, especailly know Rotate Remove Search Shift-left Equal\_range Find\_last\_if Partition return subrange.
	
	\item ranges::starts(ends)\_with, find\_last\_if and contains and contains\_subrange.
	
	\item Find(\_if)(\_not) and find\_first\_of and adjacent\_find is the basic. range add find\_last\_if\_not. std::string add find\_last\_not\_of. 
	
	\item Ranges doesn't support four math algorithms. 
	...
	\item How to use projection on stl function? can't use directly, must use your own lambda. A good example of leetcode 1637,
\begin{lstlisting}
sort(c, {}, &vector<int>::front);
//That is not correct, must use your own lambda to wrap it. 

ranges::sort(points, {}, [](auto &e ){return e.front();});		
\end{lstlisting}		
	
	
	\item ranges:: algorithm use project more, and the same time, pay attention to some algorithm return two iterators. such as search.
\end{itemize}

\subsection{Views}
\begin{itemize}
	\item Yes. In general, std::views::join\_with does not return a common\_range.
	
	\item filter is common, take\_while is not common, Structure-changing adapters (often not common\_range), such as join, join\_with, split, layz\_split, and istream. join and split are not common because the iterator has much more state than the end.
	
	This surprises many people because most containers are common ranges.
	
\begin{lstlisting}
vector<vector<int> > vv
auto vw = vv|join, //that is common
auto vw = v | chuck(2) | join, //that is NOT common
\end{lstlisting}

	\item sort vector of vector by the first element, v.front has overload, you can't use it directly, there are two methods you can over come this problem. 
...
std::ranges::sort(
    vv,
    std::ranges::less{},
    static_cast<const int& (std::vector<int>::*)() const>(
        &std::vector<int>::front));
\end{lstlisting}


	\item std::clamp (introduced in C++17) is used when you want to restrict a value to a given range.

	\item std::span is designed to be a view of contiguous memory. So you can construct a span from anything that provides a contiguous block of elements. C array  std::array std::vector Another std::span Pointer + size
	Two pointers (since C++20) Only if the iterators are contiguous iterators.
	
	\item auto pa = make\_pair(2.3, "abc"); pa type is std::pair<double, const char*> if you need string, use 
	
\begin{lstlisting}
using namespace std::string_literals;
auto pa = make\_pair(2.3, "abc"s);
\end{lstlisting}

	The other twos are chrono\_literals; and std::string\_view\_literals;
...
\hline
\textbf{No argument} & \textbf{Number} & \textbf{Lambda} & \textbf{Another range} \\ \hline
\texttt{reverse}     & \texttt{take(n)}         & \texttt{filter(pred)}       & \texttt{zip(r2)}        \\ \hline
\texttt{enumerate}   & \texttt{drop(n)}         & \texttt{transform(fn)}      & \texttt{join\_with(r2)} \\ \hline
\texttt{keys}        & \texttt{stride(n)}       & \texttt{take\_while(pred)}  & \texttt{concat(r2)}     \\ \hline
\texttt{values}      & \texttt{chunk(n)}        & \texttt{drop\_while(pred)}  &                         \\ \hline
\texttt{common}      & \texttt{slide(n)}        &                             &                         \\ \hline
\texttt{as\_const}   &                          &                             &                         \\ \hline
\end{tabular}
\caption{Common C++20/23 Range Views by Parameter Type}
\label{tab:range_views}
\end{table}	
	
	\item there are some concepts you can use judge if a view is common, size or others. 
	
	\item views::itoa(1) and ranges::to<string>() works in leetcode.
\begin{lstlisting}
string s = "12345678";

auto vw = s | views::chunk(3);
auto vw1 = vw | views::join_with(',')|ranges::to<string>();
cout<<vw1<<endl;
-std=c++23 -O2 -Wall -Wextra -Wpedantic	
\end{lstlisting}	
	
	\item filter is not SIZE, take\_while is neither COMMON nor SIZE. Can't use \texttt{auto \&} for views::transform. element\_view iterator has no -> operator
	
	\item element in views has been reference, prefer to use auto or const auto \&, don't use auto \&	
	
	\item for split or chunk\_by or slide, it return subrange, use string\_view to wrap subrange.  
	
	\item for filter, take\_while, receive a function, better to use ranges::to<string>() to change it to real container. Use ranges::to more in interview to avoid some complex problems. it's in ranges::, not views::
	
	\item adjacent pairwise, zip and enumerate all return tuple, just use auto [t1, t2, t3] to structure bind 
	
	\item error
\begin{lstlisting}
std::string num = "0123456789";

auto vw = num | std::views::stride(2);
vw = num | std::views::drop(1)  //vw has type inside, dont' reuse it. 
         | std::views::stride(2);
         
auto vw1 = num | std::views::drop(1)
          | std::views::stride(2);        
\end{lstlisting}
	
\end{itemize}

\subsection{functor and lambda}
\begin{itemize}
	\item lambda init expression and mutable
\begin{lstlisting}
...
\end{lstlisting}
	...
	\item logic(3), bit(4), math(6), compare(6), (3466)
\end{itemize}


\subsection{Bit}
\begin{itemize}
		\item n\&(n-1), clear right most, n\&-n, only keep the right most bit
...
\subsection{Tips and Precautions}
\subsubsection{Tips}
\begin{itemize}

	\item give me some examples where I can use rvalue reference
	
	This is the right question to ask after learning when copies happen.
	
	There are two different perspectives:
	
	Using rvalue references (calling code with std::move() or temporaries)
	Writing functions/classes that accept rvalue references (T\&\&).  
	
	If you write normal application code, you'll probably use std::move much more often than you'll write T\&\& yourself.
	
	\item when to use std::move?
	
This is probably the most important practical question about move semantics.

The rule is actually quite simple:

Use std::move when you are finished using an object and want to allow its resources to be transferred instead of copied.

Notice the wording:

finished using it allow transfer 

Not:

"I want it to be faster."

std::move doesn't move anything by itself—it merely says, "You may treat this object as an rvalue."	

\begin{lstlisting}
1) Passing to a sink function
oid setName(std::string name);
std::string s = "Alice";
setName(std::move(s));

2) Inserting into a container
std::vector<std::string> v;
std::string s = "Hello";
v.push_back(std::move(s));

3) Transferring ownership (unique_ptr)

4) Inside a move constructor and move assignment
class Person {
    std::string name_;
};
...
5) Local variables you're done with
std::string s = getString();
std::vector<std::string> v;
v.push_back(std::move(s));
\end{lstlisting}
	
	don't use in return, tempoary or  const objects.


	\item \textbf{first use views::, then projection, then lambda, then common algorithem. }

	\item 

	\item in order to understand where to use smart pointer, need to know where to use pointer?  know where to use rvalue reference, need to know where copy happen?  There are two level, 
	The first is RVO, it exists before rvalue reference. to resolve same problem, avoid extra copy. but rvo only helps to return. there are some other contexts need rvalue reference. 
	
\begin{table}[ht]
\centering
\begin{tabular}{|l|c|c|}
\hline
\textbf{Context} & \textbf{Copy?} & \textbf{Move Possible?} \\
\hline
Initialization          & Yes & Yes \\
\hline
Pass by value           & Yes & Yes \\
\hline
Return by value         & Yes & Yes \\
\hline
Assignment              & Yes & Yes \\
\hline
\texttt{push\_back()}   & Yes & Yes \\
\hline
Vector reallocation     & Yes & Yes \\
\hline
\texttt{std::swap()}    & Yes & Yes \\
\hline
Lambda capture          & Yes & Yes \\
\hline
Exception by value      & Yes & Yes \\
\hline
\texttt{std::sort()} internals & Yes & Yes \\
\hline
\end{tabular}
\caption{Contexts Where Copy and Move Operations May Occur}
\label{tab:copy_move_contexts}
\end{table}

\begin{lstlisting}
Case 1: Function parameters
void setData(std::vector<int> v);
setData(std::vector<int>(1000000));

Case 2: Container reallocation
This is actually one of the most important reasons move semantics were added.

Case 3: Swapping objects
template<class T>
void swap(T& a, T& b)
{
    T temp(std::move(a));
...
case 4: returning from different branches
#include <vector>
...
\end{lstlisting} 
In one word, RVO removes some moves. Rvalue references make the remaining moves cheap

\begin{figure}
\centering
\includegraphics[width=0.7\linewidth]{pics/move}
\caption{}
\label{fig:move}
\end{figure}

	
	\item We often call these sink functions.
	
	A sink function:
	
	accepts ownership,
	stores the object,
	expects the caller to be finished with it.
	
	
	forward is after rvalue reference. Perfect forwarding underpins many modern C++ facilities, such as std::make\_unique, std::make\_shared, std::thread, and the standard containers' emplace functions.
...
\end{lstlisting}
	...
	\item 
\begin{lstlisting}
std::vector<int> v = std::vector<int>(1000000);
\end{lstlisting}
...
\textbf{Task} &
\textbf{\texttt{printf}} &
\textbf{\texttt{stringstream}} &
\textbf{\texttt{std::format}} \\
\hline
String &
\texttt{\%s} &
\texttt{<< name} &
...
Float (2 decimals) &
\texttt{\%.2f} &
\texttt{std::fixed << std::setprecision(2)} &
\texttt{\{:.2f\}} \\
\hline
Hex &
\texttt{\%x} &
...
Zero padding &
\texttt{\%06d} &
\texttt{std::setw(6) << std::setfill('0')} &
\texttt{\{:06\}} \\
\hline
Type safety &
No &
...
\hline
\end{tabular}
\caption{Comparison of C++ Formatting Methods}
\label{tab:format_comparison}
\end{table}
C++98: Copy the entire vector.
C++11: Move the vector by stealing its internal pointers.
C++17: Don't create the temporary at all—construct v directly in its final location.

This progression also explains why move semantics are still essential: copy elision only applies in specific initialization and return scenarios, while moves improve performance in many other situations (function parameters, container reallocations, algorithms, swaps, assignments, etc.).

C++17 guaranteed copy elision is one of the biggest language changes since C++11. Before C++17, copy elision was an optimization. Since C++17, in certain cases it is part of the language semantics.

\begin{table}[ht]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Code} & \textbf{C++11} & \textbf{C++17} \\
\hline
\texttt{Foo f = Foo();} &
Usually elided &
Guaranteed \\
...
Guaranteed \\
\hline
\texttt{Foo f = make();} where \texttt{make()} returns \texttt{Foo()} &
Usually elided &
Guaranteed \\
\hline
\texttt{return local;} &
...
NRVO still optional \\
\hline
\texttt{return std::move(local);} &
Move &
Move (and often disables NRVO) \\
\hline
\end{tabular}
\caption{Copy Elision in C++11 vs. C++17}
\label{tab:copy_elision}
\end{table}



	\item so new FOO is just syntax candy, right? 
	
	Yes, mostly. new Foo is a piece of syntax sugar (or "syntactic sugar"), but it's a little more than just a simple function call.
	such as delete, range for and 
...
\begin{tabular}{llll}
\toprule
\textbf{Task} & \textbf{\texttt{printf}} & \textbf{\texttt{stringstream}} & \textbf{\texttt{std::format}} \\
\midrule
String             & \texttt{\%s}     & \texttt{<< name}                             & \texttt{\{\}}          \\
Integer            & \texttt{\%d}     & \texttt{<< age}                              & \texttt{\{\}}          \\
Float (2 decimals) & \texttt{\%.2f}   & \texttt{std::fixed << std::setprecision(2)}  & \texttt{\{:.2f\}}      \\
Hex                & \texttt{\%x}     & \texttt{std::hex}                            & \texttt{\{:x\}}        \\
Zero padding       & \texttt{\%06d}   & \texttt{std::setw(6) << std::setfill('0')}   & \texttt{\{:06\}}       \\
...
\end{table}
	...
	\item Most C++ programmers avoid naming varaibles
\begin{lstlisting}
min   //use minValue
max   //use maxValue
begin
...
\end{lstlisting}
	...
	\item \textbf{Most of complex data structure need hash and list combine}. 
	
	\item -std=c++23 use this flag to supports a lot of new feature. 
	
	\item Some useful patterns.
\begin{lstlisting}
for(int i = 0; auto e: container){ //loop without know the size.
	...
	++i;
}       

if( int pos = find; pos != string::npos )  //init-statement in if
                   

flag == 2 ? ++sum1 : ++sum2; //ternary

maxs = max(e, maxs); //find max

numeric_limits<int>::max()
\end{lstlisting}

	\item variable name: cnt, result, st, ans, max1, max2
	
	\item nest if can be written \&\&
\begin{lstlisting}
if(s==1){
	if(t==2){
	}
...
if(s==1 && t == 2) //a better way to express. 
\end{lstlisting}
	...
	\item dumb node at end. 
	
	\item views::chunk\_by and views::split and views::slide are very useful for some interview questions. if views::chunk\_by is not useful, then use adjacent\_find.
...
\begin{itemize}
\begin{lstlisting}
std::generate(answer.begin(), answer.end(), [&v, i = 0](){ return v[v[i++]]; }) i dones't need int

std::set<int> s(std::from_range, v); //usage of from_range
\end{lstlisting}
...
\par two methods to split.

\begin{lstlisting}
std::string str = "abc,bde,adfa";
    int init = 0;
    int pos = 0;
    while( (pos = str.find(',', pos)) != std::string::npos){
        std::string temp = str.substr(init, pos-init);
        std::cout<<temp<<std::endl;
        ++pos;
        init = pos;
    }
    std::cout<<str.substr(init)<<std::endl;
...
\par You can remember three methods by using slow and fast pointer, 
they are unique, remove and partition. in initial status, slow pointer points to what?  in unique, slow is last element of valid set, so you need to ++slow, first, then v[slow] = v[fast], in remove and partition, the slow is waiting check/invalid set the first element, so you need to v[slow] = v[fast] first, then ++slow. that is one important thing. The second thing is to use swap in partition, that is all. 

\begin{lstlisting}
//put all element>3 to the front. 
vector v{1, 1, 2, 3, 5, 4, 4, 4,4}; 
    int len = v.size();
    auto it = find_if(v, [](auto e){return e<=3;});
    int slow = distance(v.begin(), it);
    int fast = slow;
    while(fast< len){
        if(fast>3){
            swap(v[slow], v[fast]);
...
\begin{itemize}
	\item Reverse link-list (recursive and iterative methods).
...
\end{lstlisting}	
	...
	\par DFS and BFS code. For BFS, when D.pop we don't judge if it's visited.  For all next current, visit them all and push them into the D. That is the difference with DFS.
\begin{lstlisting}
DFS(start){
	S.push(start)  
	while(!S.empty()){
...
\subsection{Famous questions in leetcode}

There are four parts: 
\begin{enumerate}
	\item The labuladong.
	\item top 100/150 leetcode. 
	\item All questions above.
	\item All easy level questions below. 
\end{enumerate}


\subsubsection{Math, permutation and combination.}
\begin{itemize}
	
		\item 3010, once find min, replace with max, then do next find. flood island. Another method is to find two minimal elements. 
		
We set the first element of the array as a, the smallest element among the remaining elements as  b, and the second smallest element as c. The answer is a+b+c .
\begin{lstlisting}
...
\end{lstlisting}	
	...
	\item 2652, Given a positive integer n, find the sum of all integers in the range [1, n] inclusive that are divisible by 3, 5, or 7. Return an integer denoting the sum of all numbers in the given range satisfying the constraint. You need to know the inclusion-exclusion principle. 
...
\end{lstlisting}
	...
	\item 2928, Combination Mathematics + Principle of Inclusion-Exclusion. see the reference 
	
	\begin{lstlisting}
		https://leetcode.cn/problems/distribute-candies-among-children-i/solutions/2522970/o1-rong-chi-yuan-li-pythonjavacgo-by-end-smj5/
		
	\end{lstlisting} 
	

	\item leetcode 1539. binary search. another similar question is leetcode 41, they all need extra index information to help you to resolve the problem. 
	
	\item 2970, and 1539 are not very directly, but you need to change the question to a logic on a single element. just like a rain drop problem, need a little "building model" problem. 
	
	
%	\item k次买股票和k subset其实是一类问题。如果需要寻找最优解，那么我们需要保存中间结果，所以需要一个array[n][2][k]这样的三维数组。 如果不需要寻找最优解， 那么直接用递归就可以了。 不用array[n][2][k]保存这样的中间值。  subset子集桶角度是一个偏的树 （k*2的n次方）， 而球角度是个k叉树 (k的n次方）。这个是理解这个问题的关键。 
\end{itemize}
...
\begin{itemize}
	...
	\item 2273, use array to judge if it's anagram. don't use map here, you can use char cnt [26] directly, it's easier than map or unordered\_map.  Don't build two cnt[26] either, you can break the loop earlier, that is the common style to juge if it's a anagram. 

		
	\item 1668 and 1566 a little similar. 1566 just check pattern, I don't know what pattern content is. For 1668, we know what the pattern is. so I can use string.starts\_with
\begin{enumerate}
	\item 1566 Given an array of positive integers arr, find a pattern of length m that is repeated k or more times.
	
	A pattern is a subarray (consecutive sub-sequence) that consists of one or more values, repeated multiple times consecutively without overlapping. A pattern is defined by its length and the number of repetitions.
	
	Return true if there exists a pattern of length m that is repeated k or more times, otherwise return false.
	
	\item 1668: For a string sequence, a string word is k-repeating if word concatenated k times is a substring of sequence. The word's maximum k-repeating value is the highest value k where word is k-repeating in sequence. If word is not a substring of sequence, word's maximum k-repeating value is 0.
...
\begin{lstlisting}
    int cntRepeating(string_view sv, string word){
	int cnt{};
	int ws = word.size();
	while(sv.starts_with(word)){
		++cnt;
		sv = sv.substr(ws);
	}
	return cnt;
...
int maxRepeating(string sequence, string word) {
	...
	aaaba  should give 5, but below just give 4
	auto pos = sequence.find(word, 0);
	int ws = word.size();
	if(pos == string::npos) {
		return 0;
	}
	...
	int result{};
	auto init = pos+ws;
	while((pos = sequence.find(word, init)) != string::npos){
		if(pos - init == 0){
			++ans;
			init = pos+ws;
		}
	...
	int result{};
	while(i<n){
		auto vv = sequence | views::drop(i);
		string_view sv(vv);
		result = max(result, cntRepeating(sv,word) );
		++i;
	}
	return result;
}
\end{lstlisting}
	...
	\item leetcode 1576. look for a different characters with left and right, you can loop through "abc" and it's enough, don't need to loop through 26 character, it's a little trick. 
...
//a better version is ajacent<3> 
vector<string> findOcurrences(string text, string first, string second) {
	auto vv = text | views::split(' ') | views::adjacent<3>;
	vector<string> ans;
	for( auto const &[f, s, t] : vv){
		if(string_view{f} == first && string_view{s} == second){
...
\subsubsection{STL container}
\begin{itemize}
	
		\item 2696, A typical usage of stack.
	
	
	\item 1700 usage of stack and queue, pay attention to stack push need reverse order. use stack, that is very good idea, need to remember it. 
	\begin{lstlisting}
	...
	\item 2956, O(1) use unordered\_map, or log(n) use sort and binary search.  set operation is not suitable for this question. 
	
	\item 2006, two\_sum variant.
...
\begin{itemize}
	...
	\item 2231,  to\_string, stoi, sort, accumulate.
	\begin{lstlisting}
		int largestInteger(int num) {
			...
			string rs{};
			rs = accumulate(result.begin(), result.end(), rs, [](string s, auto e){
				s += e;
				return s;
			} );
	...
	\item 1913, max or min two, max three(nth\_element or priority\_queue?). 
\begin{lstlisting}
1) numeric\_limits<int>::max()
2) numeric\_limits<int>::min()

3) compare with max1 and max2, if bigger then max1, then max2 = max1. That is a common pattern. 
\end{lstlisting}

	\item 1636, use sort, that idea is very interesting. How to second increasing, the first decreasing.  \textbf{sort on nums directly}
\begin{lstlisting}
...
sort(v.begin(), v.end(), [](const auto& e1, const auto & e2){
	if(e1.second != e2.second){  //The second increasing
		return e1.second<e2.second;
	}
	return e1.first>e2.first; //The first decreasing.
});
...
\end{lstlisting}
	...
	\item 2465, a typical question can be resolved by the STL algorithms very efficiently. Use \texttt{std::set} to calculate the different items count.
...
int distinctAverages(vector<int>& nums) {
	...
	transform(nums.begin(), nums.begin()+nums.size()/2, nums.rbegin(), inserter(s, s.begin() ), [](auto e1, auto e2){return e1+e2;});
...
\subsubsection{fancy code}
\begin{itemize}
	
	\item Find all space, don't need while(find), use for() directly. find one max and second max, use for loop is ok, find three max, use min-heap, find three min, use max-heap. 
...
vector<string> divideString(string s, int k, char fill) {
	...
	auto v = s | ranges::views::chunk(k);
	vector<string> result;
	for(auto e : v){
		result.emplace_back(e.begin(), e.end());  //emplace_back also use very well here. 
	}
	return result;
}
\end{lstlisting}
	...
	\item 2108, use more lambda
\begin{lstlisting}
auto ll = [](const auto & e){
	int n = e.size();
	return equal(e.begin(), e.begin()+n/2, e.rbegin());
};

auto it = ranges::find_if(words, ll);
return it == words.end()? "" : *it;	
\end{lstlisting}

	\item 1816, don't need split and stringstream. A classical answer is the bottom line, after you know the bottom line, then you need to think that if there is better method to do that. 
\begin{lstlisting}
...
\begin{itemize}
	...
	\item 2190, how to get best sell book name. Pay attention to the decltype and value\_type usage. 
	
\begin{lstlisting}
auto vw = nums | views::slide(2);
...
//1) use ranges.
//2) use decltype.
//3) use value_type and projection.
auto it = ranges::max_element(um , {}, &decltype(um)::value_type::second);
return it->first;	
\end{lstlisting}	
...
string thousandSeparator(int n) {
	...
	auto vw = str| views::reverse | views::chunk(3) | views::join_with('.') | views::common;
	
	string result(vw.begin(), vw.end());
...
string reformatNumber(string number) {
	auto v1 = number | views::filter([](auto e){return ::isdigit(e);}) | ranges::to<string>();
	string result{};    
	if(v1.size()%3 == 1){
		string v2 = v1 | views::take(v1.size() - 4) | views::chunk(3)|views::join_with('-')|ranges::to<string>();
		
		string v3 = v1 | views::drop(v1.size() -4) | views::chunk(2) | views::join_with('-') | ranges::to<string>();;
		result = v2 + '-' + v3;
		
	}
	else{
		result = v1 | views::chunk(3) | views::join_with('-') | ranges::to<string>();
	}
	return result;
}
\end{lstlisting}

	\item 1108. Defanging an IP Address. if join\_with string, then change it to string\_view first.
\begin{lstlisting}
string defangIPaddr(string address) {
	string temp = "[.]";
	auto v1 = address  |  views::split('.') | views::join_with(temp);
	string s = v1 | ranges::to<string>();
}
\end{lstlisting}

	
	\item 3162, Use cartesian\_product to simulate two for loop. cartesian\_product returns the tuples. 
\begin{lstlisting}
 int numberOfPairs(vector<int>& nums1, vector<int>& nums2, int k) {
	int sum{};
	for(auto [f, s]: views::cartesian_product(nums1, nums2)){
		if(f % (s*k) == 0)
		++sum;
	}
	return sum;
...
\end{lstlisting}
	...
	\item 1662, Learn how to use views::join. Another useful function is views::join\_with can be used to add , to number. 
\begin{lstlisting}
//1) views::join usage
//2) ranges::equal 
auto vw1 = word1 | views::join; 
auto vw2 = word2 | views::join;

return ranges::equal(vw1, vw2);
\end{lstlisting}
\end{itemize}

\subsubsection{subset and subarray}
\begin{itemize}
...
\begin{lstlisting}
 std::vector<int> v1{0, 1, 2, 3, 40, 40, 41, 41, 5, 5, 5};

auto it = v1.begin();
auto pos = v1.begin();
while( (pos = std::adjacent_find(it, v1.end(), not_equal_to{}) ) != v1.end()){
	for(auto i = it; i<next(pos); ++i){
		cout<<*i<<" ";
	}
	cout<<endl;
	it = pos+1;
...
\end{lstlisting}
	\begin{enumerate}
		...
		\item chunk the digit and alphabet. 	leetcode 1805, use chunk\_by to seperate abc and number.
		
\begin{lstlisting}
string str = "123abc456efg";

auto v = str | views::chunk_by([](auto e1, auto e2){
	if(::isdigit(e1)){
		if(::isdigit(e2))
		return true;
		return false;     
...
\end{lstlisting}
	...
	\item 1863, how to get subset, how to calculate sum? calculating the only one sum, don't need calculate the subsum of each subset, you can add each item in the subset in the last sum. The second question is how to get all subset? two for doesn't work, because it's suitable for subarray. not subset. subset is pow(2, n), not n*n. Three comman ways 1) get n, build pow(2,n) bit, then use bit to select 2) recursive bucket point of view, 3) recursive ball point of view. 
...
//1) dfs should be function, auto doesn't work here.
//2) nums can use directly inside the lambda, because you have &
//3) s is subsum each subset, don't use reference here. 
//4) Base case is i ==n, then accumulate s to the last result ans. 
\end{lstlisting}

\begin{lstlisting}
void bt1(vector<int>& v, vector<int>& result, int index, int len, int xor_sum, int & sum){
	...
	*/
	...
	for(int i = index; i< len; ++i){
		...
		xor_sum^= v[i];
		bt1(v, result, i+1, len, xor_sum, sum);
...
void bt(vector<int>& v, vector<int>& result, int index, int len){
	if(index == len){
		for(auto e: result){
			cout<<e<<" ";
		}
...
\end{lstlisting}
	...
	\item 1588, dp, also subarray question, but have easy solution. 
	
	\item 2913, how to get all subarray? use two for loop can get all subarray, use set to count all different nums. 
...
\subsubsection{tree}
\begin{itemize}
	\item 108 use recursive to build a tree;
\begin{lstlisting}
...
\end{lstlisting}
	...
	\item 101, use recursive to judge mirror, just like to judge if it's same. the same idea. recursive is power weapon. 
\begin{lstlisting}
...
\end{lstlisting}
		...
		\item 530 Minimum Absolute Difference in BST. This code demonstrates how to keep previous node in BST.
	\begin{lstlisting}
		class Solution {
			public:
			TreeNode* prev;
	...
	\item 2331, no check leaf node, is it right? commonly, it's not right. only check if it's leaf code can't handle below tree. but for questions 2331, it states that it's full binary tree, so you can only check leaf code without handle base case. 
...
\item 2932. Maximum Strong Pair XOR I. Learn how to build trie tree. 

\begin{lstlisting}
	https://www.geeksforgeeks.org/maximum-xor-of-two-nodes-in-a-tree/
\end{lstlisting}
...
\subsubsection{interval}
\begin{itemize}
	\item 2848, update tail correct. That is a typical interval questions. 
\begin{lstlisting}
int numberOfPoints(vector<vector<int>>& nums) {
	ranges::sort(nums);
	int result{};
	vector<int> origin = nums[0];
	int index = 1;
	int n = nums.size();
	int tail = origin[1];
	
	for(; index <n; ++index){
		if(nums[index][0]<=tail){
			if(tail< nums[index][1]){
				tail = nums[index][1];
			}
		}
		else{
			result += tail - origin[0] +1;
			origin = nums[index];
			tail = origin[1];
		}
	}
...
\begin{lstlisting}
vector<10, bitset<3> > result;
....
\end{lstlisting}	

	
	\item 2220, how to understand \^? and how to know how many set bits in the binary represent. 
\begin{lstlisting}
bitset<sizeof(int)*8> b1(start);
bitset<sizeof(int)*8> b2(goal);
b1 ^= b2;
return b1.count();
\end{lstlisting}
	
	\item 3340, when to use stol? when not to use stol? stol to avoid overflow, how to flip flag. 
\begin{lstlisting}
bool isBalanced(string num) {
	int f[2]{};
	for (int i = 0; i < num.size(); ++i) {
		f[i & 1] += num[i] - '0';  //pay attention to i & i. 
	}
	return f[0] == f[1];
}
\end{lstlisting}	

	\item 2315, how to flip the bit? You are given a string s, where every two consecutive vertical bars '|' are grouped into a pair. In other words, the 1st and 2nd '|' make a pair, the 3rd and 4th '|' make a pair, and so forth.
...
\end{lstlisting}
	...
	\item 2032, map<int, bitset<3> > a new data structure. Because the bitset support count() function, it's very useful. 
	
	
\end{itemize}

\subsubsection{Good modeling}

\begin{itemize}
		
		\item 1854, so many detail. 1) priority\_queue(smaller) 2) how to pop up priority\_queue. That is how many meeting room questions.   
	...
	second --> first,  first--> n-first, that is all. 
		...
		\item 2873 We can use two variables mx and mxdiff to maintain the maximum prefix value and maximum difference, respectively. When traversing the array, we update these two variables, and the answer is the maximum value of all 
		
	
		\item leetcode 1566, 2765, different with 2760. 2760 can use chunk, but 2765 can't use chunk. has to use iterate method. 
	They all want to get subarray wich satisfy certain contdition. single iterate, but maintain previous condition, if condition ok, update the last result, if condition not ok, initial to the initial condition.  These three questions you need to remember, 1) how to check pattern? 2) odd and even can be used splitted so use chunk 3) increase and decrease can't be split, so use double iterate. for example 2 3 4 3  (2, 3) is answer, but 3 4 3 is also answer,  it overlop on 3. 
	\begin{enumerate}
		\item 1566 Given an array of positive integers arr, find a pattern of length m that is repeated k or more times.
		...
		\item 2760 You are given a 0-indexed integer array nums and an integer threshold.
	...
	\begin{enumerate}
		\item (start+1)\%n to simulate the circular operation.
		
		\item only need to calculate t,  s-t represents the other.
	\end{enumerate}
...
\end{lstlisting}		
	...
	\item leetcode 263, Ugly Number. An ugly number is a positive integer which does not have a prime factor other than 2, 3, and 5. Given an integer n, return true if n is an ugly number. 
		
	\item leetcode 292, Nim Game. One use or, the other use and, it will satisfy the requirement. 
\begin{lstlisting}
bool NimRecursive(int n, bool flag){
	if( flag == true&& n<=3){
		return true;
	}
	if(flag == false && n<=3){
	...
	if(flag){   
		...
		if(NimRecursive(n-1, flag ) || NimRecursive(n-2, flag ) || NimRecursive(n-3, flag ) ){
			return true;
		}
		return false;
	}
...
bool canWinNim(int n) {
	bool flag = true;
	return NimRecursive(n, flag);
}	
\end{lstlisting}

	
	\item 463. Island Perimeter, for each 1 block, only consider right and down, then -=2, it's easier than considering four sides. 
\begin{lstlisting}
...
\end{lstlisting}
	...
	\item 2506. Count Pairs Of Similar Strings. \textbf{Map 26 letter to a inter}. \textbf{words[i] consist of only lowercase English letters.} is very important clue. 
\begin{lstlisting}
int similarPairs(vector<string>& words) {
	...
	for (const auto& s : words) {
		int x = 0;
		for (auto& c : s) {
			x |= 1 << (c - 'a'); //you need to remember this common snippet
		}
		ans += cnt[x]++;
	}
	return ans;
