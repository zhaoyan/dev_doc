...
\end{lstlisting}
	...
	\item for std::array, the main interface are access, iterator and fill. std::span also has access, iterator and first, last and subspan.
...
\end{lstlisting}
	...
	\item std::erase and std::erease\_if for vector and string list and deque
\end{itemize}
...
\subsection{algorithm and pattern}

\subsubsection{useful pattern}
\begin{itemize}
	\item Three useful pattern.
\begin{lstlisting}
...
\end{lstlisting}
	...
	\item filter is not SIZE, take\_while is not COMMON. Can't use \texttt{auto \&} for views::transform. element\_view iterator has no -> operator
...
\end{lstlisting}
	...
	\item 1863, how to get subset, how to calculate sum? calculating the only one sum, don't need calculate the subsum of each subset, you can add each item in the subset in the last sum. The second question is how to get all subset? tow for doesn't work, because it's suitable for subarray. not subset. subset is pow(2, n), not n*n. Three comman ways 1) get n, build pow(2,n) bit, then use bit to select 2) recursive bucket point of view, 3) recursive ball point of view. 
...
\begin{itemize}
		\item 2231, for 0-9 digit, you can use array to count the frequency, don't need std::map. similar with counting sort. 
...
\end{lstlisting}
	...
	\item 2652, You need to know the inclusion-exclusion principle. 
