...
%		\item 所有子集， 就有两种观点，一个是球， 一个是桶。 会有两种形状的树， 但是结果一样。  桶观点是偏多叉树。 可以这么理解。 分为有1和没有1， 在没有1里面再分有2和没有2.  1， 2， 3， 4都没有， 就单独选5.  
...
%	这是一个很有代表性的题目， 貌似不能用dp求解， 只能 back track.
...
\begin{itemize}
	...
	\item \textbf{The basic question can be asked: can(bool), min or count(one number), all(list or vector)}
...
\begin{itemize}
	...
	No Aftereffect (or Memorylessness)
	This has two meanings. The first is that when deriving the state of a later stage, we only care about the state values of the earlier stages, not how those states were derived step by step. The second meaning is that once the state of a certain stage is determined, it is not affected by decisions made in later stages. No aftereffect is a relatively "relaxed" requirement. As long as the dynamic programming problem model mentioned earlier is satisfied, the no aftereffect property will generally hold.
	...
	\item 
Dynamic Programming
The general form of a dynamic programming problem is to find the optimal solution (e.g., minimum or maximum value). Since we are looking for an optimal value, what is the core problem? The core issue in solving a dynamic programming problem is exhaustive search. The process involves clarifying the base case, identifying the "states," determining the "choices," and defining the meaning of the dp array or function. I summarize this part of the theory as "one model and three characteristics."
First, let's look at what "one model" means. It refers to the model for problems suitable for dynamic programming. I define this model as the "multi-stage decision-making optimal solution model." Let me explain this in more detail.

We typically use dynamic programming to solve optimization problems. Solving such problems requires passing through multiple decision-making stages, where each stage corresponds to a set of states. Then, we look for a sequence of decisions that can produce the final optimal value we are seeking.

Now, what are the "three characteristics"? They are optimal substructure, no aftereffect, and overlapping subproblems. These concepts may seem abstract, so I will explain each in detail.

Optimal Substructure
The optimal substructure means that the optimal solution to a problem contains the optimal solution to its subproblems. In other words, we can derive the optimal solution of the main problem from the optimal solutions of its subproblems. If we map the optimal substructure to the dynamic programming model we defined earlier, we can understand that the states in later stages can be derived from the states in earlier stages.

No Aftereffect
No aftereffect has two layers of meaning. The first is that when deriving the state of a later stage, we only care about the state value of the earlier stage, not how that state was derived step by step. The second layer is that once the state at a certain stage is determined, it will not be affected by decisions made in subsequent stages. No aftereffect is a fairly "relaxed" requirement. As long as the dynamic programming model mentioned earlier is satisfied, it will typically satisfy the no aftereffect property.

Overlapping Subproblems
This concept is relatively easy to understand. As mentioned multiple times earlier, in dynamic programming, different decision sequences may lead to the same stage, potentially resulting in overlapping states.

Example
Identify the base case
This is simple. Clearly, when the target amount is 0, the algorithm should return 0, because no coins are needed to achieve the target amount.

Identify the "states"
These are the variables that change between the main problem and subproblems. Since the number of coins is unlimited and the coin denominations are given in the problem, the only thing that changes is the target amount as it approaches the base case. Therefore, the only "state" is the target amount.

Identify the "choices"
These are the actions that cause the state to change. Why does the target amount change? Because you are choosing coins. Every time you choose a coin, the target amount decreases by the coin's value. Therefore, all the coin denominations represent your "choices."

Define the dp function/array
We are discussing a top-down approach, so there will be a recursive dp function. Typically, the function's parameters are the variables that change during state transitions (i.e., the "states" mentioned above), and the function's return value is what we need to calculate. For this problem, the only state is the target amount, and we are asked to calculate the minimum number of coins required to reach that amount.

Additional Insights
This isn't exclusively a dynamic programming problem, but it demonstrates that the optimal substructure is not unique to dynamic programming. Most problems involving optimization (finding the best solution) possess this characteristic. Conversely, the optimal substructure property is a necessary condition for dynamic programming problems, so when you encounter an optimization problem, think in terms of dynamic programming. This is the basic "template."

Steps in Dynamic Programming:
The first step is to break the current problem into subproblems. Once it's broken down, define the state transition equation—that is, how the answer to the current problem is derived from the combination (transition) of the subproblems.

Decide whether to use top-down dp (subproblem, subproblem state 1, subproblem state 2) or bottom-up dp (dp[subproblem], [subproblem state 1], [subproblem state 2]).

Based on the meaning of dp, determine the direction of traversal and the base case.
...
%	面试的时候优先用top down, 递归方式。 关键是遍历所有选项，根据选项改变状态后， 把新状态传入子问题。 然后在对结果进行处理，有点后序遍历的味道。 这一点很重要。
	...
	\end{lstlisting}
				% comment above return //选项有两个，所以不用写for了， 根据选项改变了状态，然后把状态传入递归调用，返回的结果进行max. 
