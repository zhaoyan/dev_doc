...
\subsection{Basic data and algorithms}


\begin{itemize}
	\item Circular queue: 1) use \%, 2) when is full? 3) when is empty? 
	
	\item reverse(double pointer +swap) and rotate(based on reverse).
...
-3%2   equal -1	
\end{lstlisting}	
	know gcd and lcm. How to get prime number. 
...
\begin{itemize}
	...
	\item You can use contains and starts\_with and ends\_with if compiler supports. 
	
	\item string and other a few other range-like, such as std::span and std::array.
	...
	\item how to construct? use vector.reserve. 
\begin{enumerate}
	\item iota
...
Prefer to use range and reserve
\begin{lstlisting}
vector v1 {1, 2, 3};
vector v2;
...
for map, insert, emplace, emplace\_hint, try\_emplace difference. [] and insert\_or\_assign difference. Use more emplace if it is possible. 

for different container, insert has different overload, for sequence, need a position for insert position, for associate, usually doesn't need insert position, but have a insert(hint, value)to keep consistent with sequence insert. Usually they all support range insert(iter1, iter2).

erase+remove for vector, remove for list and erase for associatate. 

std::erase and std::erease\_if for vector and string and deque

fill and generate can be thought as another insert. 

remove a value or a value which satisfy a condition(\_if) in vector, list and map.
...
find, count, contains, find\_if,  low/upper\_bound and binary\_search

all/none/any\_of return bool. 

\subsubsection{operations}

basic shuffle and sort operation. 
...
\begin{itemize}
	...
	\item to\_string and stol, stof, stoul
...
\begin{lstlisting}
for(int i ; auto e: container){ //loop without know the size.
	...
	++i;
}                          
