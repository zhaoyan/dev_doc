...
int main() 
{
	...
	std::vector< std::pair<int, int> > data(dic.begin(), dic.end());
	...
	vector vi = {1, 2,3, 4, 5, 6, 3, 5 };
	auto len = vi.size();
	for( auto i = 0;i<vi.size()/2; ++i){
		swap(vi[i], vi[len-1-i]);
	}
	
	int i = 2;
	cout<<count_if(vi.begin(), vi.end(), [&i](auto e) {return e>i;} )<<endl;
	...
	vi.erase(remove_if(vi.begin(), vi.end(), [](auto i){return i%2==0;}), vi.end());
